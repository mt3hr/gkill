...
**`getAllTags` は「タグ無し」検索のときだけ走らせる:** `find_filter.go` の `getAllTags` は**全repの全タグ**を集めて `RelatedTagIDs`（＝タグが1つでも付いているIDの集合）を作る。実データでは1検索あたり数十万行になる重い処理だが、**読み手は `filterTagsKyous` / `filterTagsTimeIs` の `NoTags`（`"no tags"` 仮想タグ）分岐しか無い**（OR・ANDそれぞれ2箇所）。したがって起動条件は `containsNoTags(Tags) || (HasTimeIsFilter() && containsNoTags(TimeIsTags))`。以前は `Tags != nil` だったので、rykv の既定クエリ（`tags` が常に非null）ではタグを明示的に選んだだけの検索でも毎回まるごと無駄に走っていた。**壊れ方が静かなので両方向を固定してある** ―― 走らせ忘れると `RelatedTagIDs` が空になって**全件が「タグなし」扱い**になり、タグの付いた記録まで返る。回帰は `get_kyous_tag_filter_test.go` と `get_kyous_regressions_test.go` の `TestHandleGetKyous_TimeIsTagsFilterWorksWithoutKyouTagFilter`。`NoTags` の照合は `containsNoTags` も各分岐も**完全一致**（片方だけ大小無視にすると「集合を作らないのに NoTags 分岐へ入る」＝常に0件になる）。
