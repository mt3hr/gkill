...
**Test coverage (~1,320+ tests total, all 29 Go packages covered):**
...
Key packages:
- `gkill/api/` — HTTP API handlers via gorilla/mux, **77 POST endpoints** under `/api/`. Main handler: `gkill_server_api.go` (~14,000 lines). Route definitions: `gkill_server_api_address.go`.
- `gkill/api/embed.go` — `//go:embed embed` directive serves the compiled Vue SPA at root `/`
- `gkill/api/req_res/` — Request/response structs for every endpoint (172 types)
