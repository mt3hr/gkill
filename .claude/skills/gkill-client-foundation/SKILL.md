...
- `pages/views/` — 207 view components, `pages/dialogs/` — 119 dialog components (Escape key closes via `useFloatingDialog`), including ZIP contents browser, plugin HTML views (`plugin-html-view.vue`, `plugin-html-context-menu.vue`, `plugin-config-dialog.vue`), and Dnote trend/correlation graph components (client-side aggregation, no server API)
- `classes/api/gkill-api.ts` — Singleton `GkillAPI` class (~3,600 lines), client-side API wrapper
...
**一覧の行のクリックは `v-list-item` に付ける:** `v-list-item-title` は行の中の文字1行ぶん（48px の行に 24px、面積で4割）しか無く、`@click` をそちらに付けると上下12px・左右16pxの余白を押したときにリップルが出てメニューが閉じるだけで何も起きない。画面切替メニュー8画面ぶんが全部この形で、APK（指で押すスマホ）で「メニューを押しても遷移しない」と報告された（2026-09-24）。`convention-source-scan.test.ts` が走査で守る。
