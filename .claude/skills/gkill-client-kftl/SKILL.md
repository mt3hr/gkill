...
**KFTL 経由で書いた記録は `create_app="gkill_kftl"` / `create_device=<サーバのdevice>`。** MCP から書いても同じで、手打ちのメモ帳と**区別する欄が無い**（`gkill_add_*` は `gkill_mcp_readwrite` / `mcp`）。`create_apps:["gkill_mcp_readwrite"]` を「MCP で作った記録の探し方」と案内しないこと。例外は Wear OS で、companion が `SubmitKFTLTextRequest` の任意項目 `create_app` に `gkill_wear` を載せるので `create_app="gkill_wear"` になる（2026-09-11〜。サーバは空なら `gkill_kftl` に落とす。詳細は [gkill-mobile](../gkill-mobile/SKILL.md)）。失敗した送信は何も残らない（確定は `CommitTx` の1つのトランザクション、ADR-0219。応答の `created` は `[]` で `null` にしない）のでそのまま送り直せばよいが、**成功した送信の再送**（応答だけ落ちた等）を畳むために `idempotency_key` を同じ値で渡す（受け口は `SubmitKFTLTextRequest`。渡さないと丸ごと二重登録になる）。**台帳はキーだけでなく本文の指紋（本文 + `create_app` の SHA-256）と `created[]` を控える**（`kftl_idempotency.go`、TTL 10分）。同じキー・同じ本文なら元の `created[]` を `replayed:true` で返し（応答を失った側が ID を回収できる）、同じキーで別の本文なら `ERR000423`（409）で何も書かない。2026-09-19 までキーしか見ておらず、別の本文にも「成功・`created` 空」を返して本文は保存されていなかった。**指紋を持たない形へ戻さない・本文そのものを台帳に置かない**（[ADR-0510](../../../documents/adr/0510-kftl-idempotency-key-carries-fingerprint-and-result.md)）。守るテスト: `handle_submit_kftl_text_test.go` の `TestHandleSubmitKFTLText_IdempotencyKey` / `TestHandleSubmitKFTLText_CreatedIsEmptyArrayOnFailure`。
