...
|---|---|---|---|---|
| Read | 12 (status, get_mcp_help, get_kyous, get_mi_board_list, get_all_tag_names, get_all_rep_names, get_gps_log, get_application_config, get_rep_infos, get_idf_file, get_kyou_history + plugin 1) | `npm run mcp:gkill-read` | `npm run mcp:gkill-read-http` | 8808 |
| Write | 29 (9 add + 1 submit_kftl + 1 delete + 1 restore + 9 update + 7 read convenience + plugin 1) | `npm run mcp:gkill-write` | `npm run mcp:gkill-write-http` | 8809 |
| ReadWrite | 33 (read 11 + write 21 + plugin 1) | `npm run mcp:gkill-readwrite` | `npm run mcp:gkill-readwrite-http` | 8810 |
...
**ツール一覧の世代は `gkill_status` の `schema_revision` で見せる。** ツール一覧はクライアントのセッション寿命で固定される（後述）ので、サーバのプロセスが新しくても、クライアントが握っている一覧は古いままでありうる。2026-09-14 のレビューで ChatGPT が改名前の検索条件名を一覧どおりに送り「未知の引数」で拒否された件は、NSSM の MCP サービスが改名後（2026-09-10）に起動していたのに起きた —— **古かったのはサーバでなくクライアント側のスナップショット**で、サーバ再起動では直らず、コネクタの接続し直しが要る。この切り分けを AI 自身にさせるため、`schema_revision`（そのサーバが配る tools 配列から `gkill_status` を除いた JSON の SHA-256 先頭12桁。`lib/status-tool.mjs` の `computeSchemaRevision`）を **`gkill_status` の description 末尾に `[schema_revision: xxxx]` として焼き込み**、応答の `schema_revision` と比べさせる。焼き込みは `McpServerBase` のコンストラクタが `stampSchemaRevision` で行い、静的な `READ_TOOLS` は書き換えない（3サーバで値が違う）。`initialize` の `serverInfo.version` は `<package.json の version>+schema.<revision>`、起動ログ `server_start` にも `schema_revision` / `tool_count` / `pid` が載るので、「ソースは直っているのに AI からは古い」は、まずログの revision と `gkill_status` の応答と、AI が引用する description の印を見比べる。**手書きの版番号にしない**（更新し忘れる）。**`gkill_status` に引数を足さない**（`STALE_SCHEMA_ARG_KINDS_BY_TOOL` の対象になり、古さを確かめるツール自身が古い一覧で壊れる）。「read と readwrite の同名ツールは同じ定義」の唯一の例外がこの焼き込みで、`schema-contract.test.mjs` は印を剥がしてから3サーバの tools/list を比較する（[ADR-0619](../../../documents/adr/0619-mcp-schema-revision-and-stale-tool-list.md)）。

**未知の引数名には「古い一覧の可能性」を必ず添える。** 書き間違いと、改名・削除前の名前を載せた古い一覧とは、サーバからは区別できない。文言の正本は `lib/validation.mjs` の `unknownKeyMessage()` 1本で、`assertKnownKeys`（トップレベル）と `normalizeKyouQuery`（`query` の中）の両方がそれを使う —— 両方の可能性を言い、再接続と `gkill_status` の照合を案内する。**改名前の名前を受理する互換は書かない**（旧綴りは `verify_docs` が禁止する。ADR-0806）。「スキーマにある名前を正規化器が知らない」側の事故は `schema-contract.test.mjs` が防ぐ: スキーマのキー集合 = 受理集合 − 廃止済み、各ツールを全プロパティ指定で呼んで未知キーで落ちない、3サーバの同名ツールが同じ JSON。

**tools/list のバイト量は予算ファイルで固定する。** 事故対策を説明文へ足すたびに一覧は太り、合計だけが誰にも見えていなかった（2026-09-14 時点で readwrite 約 94KB、`gkill_get_kyous` 単体 27KB）。予算は `src/mcp/tool-schema-budget.json`（現状の実測。閾値は頭で決めない）で、`tool-schema-budget.test.mjs` が3サーバの実測を突き合わせる: **超えたら失敗、1024 バイト以上下回っても失敗**（減ったら予算を追随させ、緩んだ予算が次の増加を吸収しないようにする）。更新は `npm run mcp:schema-budget -- --update` で、増やしたときはコミットメッセージに理由を書く。計測と判定の正本は `src/mcp/tool-schema-budget.mjs`（サーバ3本を import するので `lib/` には置かない）。

**ツール一覧の説明文は要約にとどめ、本文は `gkill_get_mcp_help` の topic に置く。** 事故対策を説明文へ足し続けた結果、readwrite の tools/list は約 94KB、`gkill_get_kyous` 単体 27KB、`gkill_submit_kftl` の説明文だけで 7.5KB になり、AI はタスクに関係ない説明まで毎回読んでいた（2026-09-18 の実利用報告）。説明文に残すのは「何をするか・まず使う引数・必ず確認すること・詳細の在処」で、本文は `lib/help-topics.mjs` の topic（search / pagination / mi / data_types / plugin / idf / deleted / rep / kftl）へ**要約せずに移す**。`gkill_get_mcp_help(topic)` は3サーバ共通（`gkill_status` と同じく `WRITE_SERVER_READ_TOOL_NAMES` にも載せる）で、gkill へは往復しない。**説明文へ事故対策を足したくなったら、まず topic へ足すこと。** 要約側へ足すのは「必ず確認すること」に昇格する場合だけで、予算テスト（`tool-schema-budget.test.mjs`）が増加を止める。移した知識の実在（KFTL の `~~` / `??` / `/endt?`、Mi の射影、warnings の規則 …）と本文が名指しするツール名の実在は `help-topics.test.mjs` が固定する（説明文の名指し走査は本文には掛からない）。要約化した説明文が在処を案内していることは `tool-handlers.test.mjs` が固定する（[ADR-0622](../../../documents/adr/0622-tool-descriptions-are-summaries-details-via-help-tool.md)）。実測: read 47,187 → 36,109 バイト、readwrite 94,325 → 77,529。
