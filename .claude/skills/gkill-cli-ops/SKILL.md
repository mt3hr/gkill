...
Both use cobra for CLI with shared subcommands: `version`, `dvnf`, `generate_thumb_cache`, `generate_video_cache`, `optimize`, `update_cache`, `clear_cache`, `generate_plugin_cache`, `reset_password`, `add_tag`. `idf` is registered by `gkill_server` only (`main/gkill/main.go` does not register `IDFCmd`). Default listen address: `127.0.0.1:9999` with "local access only" on (loopback-only; the constants live in `dao/server_config/server_config.go`, see [gkill-go-backend](../gkill-go-backend/SKILL.md)), TLS disabled, initial user `admin` with no password. Serving other machines on the LAN is an explicit change in the server settings screen (address + local-only), not a default.
