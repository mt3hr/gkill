...
- **肯定語は「対象列に含む OR ID が語で始まる」（ID は前方一致だけ）。除外語は「対象列に含まない」で ID は見ない。** 部分一致だったころは `1` / `a` / `cafe` のような hex だけの短い語が UUID に偶然含まれ、無関係な記録が「ランダムに」出て、`-1` が無関係な記録を消していた（1文字なら ~86% の UUID が当たる）。除外語を肯定語として再検索する内部クエリは `FindQuery.WordsSkipIDMatch`（JSON に出ない）を立て、Go 側は id に空文字を渡す
