...
**Android同梱サーバの待受・TLS・画面のアドレスは ServerConfig に従う**（2026-09-24、[ADR-1104](../../../documents/adr/1104-android-server-follows-server-config.md)）。`MainActivity` は `--address` / `--disable_tls` を渡さず、Kotlin にポートもスキームも持たない。WebView の URL は、サーバが標準出力に出す `Access your record space at : ` 行（`close.go` の `PrintStartedMessage`。`ENABLE_THIS_DEVICE` 行の ADDRESS のポートと ENABLE_TLS から `localhost` で組む。**文言を変えないこと**）だけから取る。サーバ設定を保存すると行が出直すので、オリジンが変わったときだけ開き直す。**上書きのフラグや 9999 を Kotlin に書き戻さないこと** ―― 設定画面の待受アドレスと TLS が Android でだけ黙って効かなくなる（2026-09-24 まで `--address 127.0.0.1:9999 --disable_tls` で実際にそうだった）。新規の置き場の既定は `127.0.0.1:9999` ＋「ローカルアクセスのみ許可」なので既定では LAN に出ないが、古い設定 DB や PC から写した設定 DB は ADDRESS 次第で LAN に開く。ADDRESS に特定の LAN IP だけを書くとループバックで待ち受けず、WebView（`localhost`）から届かない。TLS 有効時の自己署名証明書は `onReceivedSslError` がループバックに限って通す。既存サーバの再利用（先行プローブ）は前回拾った URL のポートで行う。activity に `configChanges`（回転で Activity を再生成させず、SQLite書き込み中の `kill -9` を防ぐ）と `onBackPressedDispatcher`（WebView goBack）。cleartext は `network_security_config.xml` で localhost 限定、外部ストレージ権限は `maxSdkVersion` 付き。データ置き場（`--gkill_home_dir`）は `/sdcard/gkill`（2026-09-24 にアプリ専用領域から戻した）なので、**起動ゲートは共有ストレージへの権限が許可されるまでサーバを起動しない**（M-15 の非ブロッキング化はデータが専用領域にあることが前提だった）。権限の要求画面を自動で出すのは Activity ごとに1回だけで、以後は画面のボタンから出す（onResume のたびに出すと設定画面から抜けられない）。専用領域にデータを置いていた版からの複製 `copyAppPrivateHomeIfNeeded` は `/sdcard/gkill` が無いか空のときだけ行い（中身があれば `/sdcard/gkill` を正とし、専用領域は消さない）、一時ディレクトリへ複製してから改名する。**複製に失敗したらサーバを起動しないこと** ―― 起動すると gkill_server が空の置き場を作り、次回から「中身あり」と見なされて専用領域のデータへ二度と戻れない（守るテストは `MainActivityUnitTest.kt`）。**Wear companion の TLS は TOFU/ピン留め**（H-05）: `GkillServerTrust.kt` がプラットフォーム既定で検証し、失敗時のみ保存済み SHA-256 フィンガープリント一致で許可（trust-all は全廃）。ピン学習は companion の「保存&接続テスト」で利用者承認時のみ。ウォッチ→電話の送信は `WearRequestWorker`（WorkManager）で Service 破棄を跨ぎ、`WearSubmitLedger` で重複再配送を確認へ回す（S3-wear）。KFTL送信にはサーバ側冪等キーも付く: `GkillWearableListenerService` がメッセージ1件ごとに UUID を採番して WorkRequest の不変入力に載せるので、同じ要求のワーカー再送では同じキーになり `handle_submit_kftl_text.go` の `kftlIdempotencyStore`（TTL 10分・成功時のみ記録。本文の指紋と `created[]` も控え、同じ本文なら元の `created[]` を `replayed:true` で返し、別の本文なら `ERR000423`（409）で何も書かない。ADR-0510）が二重登録を畳む。意図的な再送は別メッセージ＝別キーなので畳まれない。**冪等キーを内容ハッシュにしないこと**（意図的な同一内容の再送が畳まれて記録できなくなる）。**ハンドラでの `markDone` 配線を落とさないこと** ―― ストア単体テストは `markDone` を直接呼ぶので配線漏れを見逃す（ビルドも vet も素通しする＝`idempotencyKey` は `alreadyDone` 分岐で使われ未使用にならない）。守るテストは `kftl_idempotency_test.go`（ストア単体）/ `handle_submit_kftl_text_test.go`（2回叩いて2回目が畳まれる end-to-end。同一キー=1件・別キー=2件・キー無し=2件）/ `WearRequestHandlerTest.kt`。

**同梱サーバのタイムゾーンは Go と libc の両方へ教える**（2026-09-16、[ADR-0220](../../../documents/adr/0220-sqlite-localtime-follows-libc-zone.md)）。Go の `time.Local` は Android では常に UTC で、`main/common/fix_timezone.go` の `fixTimezone` が `getprop persist.sys.timezone` で直す。SQLite の `'localtime'` は Go ではなく libc（musl 転写）が `TZ` / `/etc/localtime` から決め、Android にはどちらも無いので UTC のままになる。`applyLibcTimezone` が端末の packed tzdata（`/apex/com.android.tzdata/etc/tz/tzdata` 等）から TZif を切り出して `$GKILL_HOME/tz/localtime` に置き、`TZ=:<パス>` を**最初の SQLite 接続より前**（`InitGkillOptions` の末尾）に入れる。tzdata が読めなければ POSIX 固定オフセット（`JST-9`）へ落ちる。**Go 側だけ直して libc 側を放置してはいけない** —— 時間帯フィルタが黙って0件になる。**`applyLibcTimezone` へ渡すパスは環境変数を展開した絶対パス** —— `gkill_options.GkillHomeDir` の既定値は `$HOME/gkill` の未展開文字列で、そのまま渡すと `TZ=:$HOME/...` のリテラルになり、musl は相対名を zoneinfo ディレクトリで探して無ければエラーなしで UTC にする（Termux は `--gkill_home_dir` 無しで起動するので踏む。APK は `MainActivity` が絶対パスを渡すので踏まない。2026-09-16 同日に発覚）。`InitGkillOptions` が展開済みを渡し、`applyLibcTimezoneFor` 側でも `filepath.Abs` して絶対パスにできなければ POSIX 文字列へ落ちる。**`TZ` を `InitGkillServerAPI` より後で入れてはいけない** —— modernc の libc は最初の接続で `os.Environ()` を写し取り、以後の `os.Setenv` を見ない。Android アプリ側（`MainActivity` の `ProcessBuilder`）で `TZ` を渡す案は Termux 等の手動起動に効かないので採らない。守るテストは `main/common/fix_timezone_test.go`（packed tzdata の切り出し・`TZ` の組み立て・fallback）と `dao/sqlite3impl/localtime_check_test.go`。

**ステータスバーは gkill のテーマ色で塗り、API 34 以下と 35 以上の2経路をそろえる**（2026-09-24）。色は `colors.xml` の `gkill_indigo`（Web の primary と同じ値）。API 34 以下はテーマの `android:statusBarColor` と `android:windowLightStatusBar=false` が塗る ―― **`values` と `values-night` の両方に書くこと**（夜間用の `Base.Theme.Gkill` は丸ごと差し替わり、昼の項目を継がない）。API 35 以上は edge-to-edge が強制されて `statusBarColor` が黙って無視されるので、`activity_main.xml` のステータスバーの裏の帯 `status_bar_background` を `MainActivity` の insets リスナがステータスバーの高さに合わせる ―― **`layout` と `layout-sw600dp` の両方に置くこと**。システムバーぶんの余白は `content_container` のマージンで取る（`android.R.id.content` にパディングを付けると帯ごと押し下がって塗れない）。守るテストは `MainActivityUnitTest.kt` の `statusBarColor_matchesWebPrimary`（`vuetify.ts` の primary との突き合わせ）/ `statusBar_isPaintedWithGkillIndigoInAllThemesAndLayouts`。
...
Kmemo が2件書かれる。**関連時刻の行を落とさないこと** ―― `WearSubmitLedger` はテキスト完全一致・TTL30分で
重複を判定するので（2026-09-21 まで24時間。同じ打刻テンプレートを半日後に押し直すだけで毎回確認が出ていた。
窓は最後に保存した時刻から数える）、時刻が無いと「30分内の2回目の同じ気分値」が `DUPLICATE` になる。
