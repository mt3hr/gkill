...
Android 同梱サーバはこのフラグも `--disable_tls` も付けずに起動するので、待受アドレス・TLS・アプリの画面が開くアドレスは
この端末の設定行（ADDRESS / ENABLE_TLS）に従う（[ADR-1104](../adr/1104-android-server-follows-server-config.md)）。
