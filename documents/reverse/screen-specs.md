...
| `/mkfl` | mkfl-page | ファイル管理 | ファイル・メディア管理 |
| `/plaing` | plaing-timeis-page | 打刻一覧 | アクティブな打刻セッション |
| `/saihate` | saihate-page | ダッシュボード | メイン画面・各種記録の追加 |
...
**コンポーネント:** `mkfl-page.vue` → `mkfl-view.vue`

ファイル（画像、動画等）の管理画面です。

| 項目 | 種別 | 説明 |
|---|---|---|
| ファイル一覧 | 表示 | ファイル記録のリスト表示 |
| ファイルアップロード | 操作 | ファイル追加 |
| ファイル編集 | 操作 | メタデータ編集 |
| 日時関連付け | 入力 | アップロードファイルの記録日時指定 |
...
### 2.8 ダッシュボード画面（`/saihate`）

**コンポーネント:** `saihate-page.vue`

メインダッシュボードです。フローティングアクションボタン（FAB）から各種記録を素早く追加できます。
