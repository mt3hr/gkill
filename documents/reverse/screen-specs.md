...
| ビューコンポーネント | 207 |
| ダイアログコンポーネント | 121 |
| **コンポーネント合計** | **343**（ルートを除く。ビュー207 + ダイアログ121 + ページ15） |
...
| 相関グラフ | `dnote-correlation-graph-view.vue` | 2～10指標のPearson／Spearman相関をヒートマップと散布図で表示。共通粒度と方向付きlagを設定可能。指標ごとに「記録が無い期間を0とみなす」（件数・合計のみ）と「日をまたぐ打刻の計上先」（分割／開始／終了）を選べる。行列は7指標まで400px幅の列に横スクロールなしで収まる（トラック幅の根拠は `classes/dnote-correlation-matrix-layout.ts`。8指標以上は行列だけが横スクロールする） |
...
|---|---|---|
| 店名 | 入力 | 店名（Shop）。全行で共有 |
| タイトル | 入力 | 支出の内容（Title）。行ごと |
| 金額 | 入力 | 金額入力（Amount）。行ごと |
| 行の削除 | 操作 | その行を消す（最後の1行は消せない） |
| 追加 | 操作 | タイトルと金額の行を1つ足す（既定は1行） |
| 日時 | 入力 | 記録日時。全行で共有 |

並びはメモ帳（KFTL）の支出と同じく、店名 → タイトルと金額の行。1回の保存で行の数だけ支出の記録ができ、
タグ欄のタグは全行に付く。全行を1つの tx で書くので、1件でも失敗すれば何も残らない。
編集画面（`edit-nlog-view.vue`）は1件ずつで、並びは店名 → タイトル → 金額。
