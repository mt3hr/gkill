...
> （実データで十数個のキャッシュrepが数百個の生repに展開され、git だけで1窓あたり十数秒）。
...
同一プラグインへは必ず1件ずつ直列に投げる。gkill 側は1プラグイン1並列なので
同時発行しても順番待ちが伸びるだけで、待ちきれなかったぶんは `ErrPluginBusy` になる。
呼び出し元のキャンセルではプロセスは落ちない。回収するのは gkill 自身のデッドラインだけ。

（2026-08-06以前は期限を排他ロックの前に張っていたため、ロック待ちで期限を食い潰し、
期限切れ時の `Process.Kill()` でプラグインプロセスが回収されていた。
MCPの読み取りが原因でユーザーのプラグインが落ちるのはこれが理由だった。）
