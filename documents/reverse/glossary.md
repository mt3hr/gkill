...
| **GkillServerAPI** | HTTP API ハンドラ。gorilla/mux で全エンドポイント（92定義・90登録）を提供する。`gkill_server_api/` パッケージ（handle_*.go 102ファイル）に分割実装 |
...
| **MCP サーバ** | AI 統合用 MCP サーバ。3バリアントが存在する。**Read専用**（`gkill-read-server.mjs`、11ツール）・**Write専用**（`gkill-write-server.mjs`、26ツール）・**ReadWrite統合**（`gkill-readwrite-server.mjs`、32ツール）。いずれも共通のプラグインツール1つ（`lib/plugin-tools.mjs` の `PLUGIN_TOOLS`）を含む。各バリアントは stdio（ローカル）/ HTTP（OAuth 2.1付きリモート）の2モードをサポート |
...
| APIハンドラ（個別） | `src/server/gkill/api/gkill_server_api/handle_*.go` | 個別エンドポイントのハンドラ（handle_*.go 102ファイル、1ハンドラ1ファイル） |
...
| エラーコード定義 | `src/server/gkill/api/message/error_codes.go` | ERR000001〜ERR000415 の定数定義（計411件。ERR000243は欠番） |
...
| ロケールファイル | `src/locales/*.json` | ja, en, zh, ko, es, fr, de（922キー/言語） |
