...
| **MCP サーバ** | AI 統合用 MCP サーバ。3バリアントが存在する。**Read専用**（`gkill-read-server.mjs`、12ツール）・**Write専用**（`gkill-write-server.mjs`、29ツール）・**ReadWrite統合**（`gkill-readwrite-server.mjs`、33ツール）。いずれも共通のプラグインツール1つ（`lib/plugin-tools.mjs` の `PLUGIN_TOOLS`）を含む。各バリアントは stdio（ローカル）/ HTTP（OAuth 2.1付きリモート）の2モードをサポート |
...
| 概念 | ファイルパス | 説明 |
|------|-----------|------|
| MCP サーバー（Read） | `src/mcp/gkill-read-server.mjs` | 読み取り専用MCPサーバー（12ツール = 固有11 + プラグイン1、stdio/HTTP） |
| MCP サーバー（Write） | `src/mcp/gkill-write-server.mjs` | 書き込み専用MCPサーバー（29ツール = 書き込み21 + Read便利7 + プラグイン1、stdio/HTTP） |
| MCP サーバー（ReadWrite） | `src/mcp/gkill-readwrite-server.mjs` | 読み書き統合MCPサーバー（33ツール = 固有32 + プラグイン1、stdio/HTTP） |
