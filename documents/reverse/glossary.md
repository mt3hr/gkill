...
| **GkillServerAPI** | HTTP API ハンドラ。gorilla/mux で全エンドポイント（90定義・88登録）を提供する。`gkill_server_api/` パッケージ（handle_*.go 94ファイル）に分割実装 |
...
| APIハンドラ（個別） | `src/server/gkill/api/gkill_server_api/handle_*.go` | 個別エンドポイントのハンドラ（handle_*.go 94ファイル、1ハンドラ1ファイル） |
