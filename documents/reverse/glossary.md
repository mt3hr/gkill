...
| **GkillServerAPI** | HTTP API ハンドラ。gorilla/mux で全エンドポイント（80）を提供する。`gkill_server_api.go`（約570KB）に集約 |
...
| **MCP サーバ** | gkill-read-server.mjs。AI 統合用の読み取り専用 MCP サーバ（7ツール提供: gkill_get_kyous, gkill_get_mi_board_list, gkill_get_all_tag_names, gkill_get_all_rep_names, gkill_get_gps_log, gkill_get_application_config, gkill_get_idf_file） |
...
| 概念 | ファイルパス | 説明 |
|------|-----------|------|
| APIエンドポイント定義 | `src/server/gkill/api/gkill_server_api_address.go` | 全80エンドポイントのパス・メソッド定義 |
...
| リクエスト/レスポンス型 | `src/client/classes/api/req_res/` | TypeScript 版入出力型（150ファイル） |
...
| ロケールファイル | `src/locales/*.json` | ja, en, zh, ko, es, fr, de（~765キー/言語） |
...
| ビルド設定 | `package.json` | npm scripts、依存関係、バージョン (1.1.3-dev) |
