...
| **GkillServerAPI** | HTTP API ハンドラ。gorilla/mux で全エンドポイント（92定義・90登録）を提供する。`gkill_server_api/` パッケージ（handle_*.go 97ファイル）に分割実装 |
...
| APIハンドラ（個別） | `src/server/gkill/api/gkill_server_api/handle_*.go` | 個別エンドポイントのハンドラ（handle_*.go 97ファイル、1ハンドラ1ファイル） |
