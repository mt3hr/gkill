...
├── resources/              # リソースファイル
│   ├── manual/             # 生成済みHTMLマニュアル（7言語×24ページ、go:embed対象。gitignore済みで build_manuals が生成する）
...
├── gkill-write-server.mjs     # Write専用MCPサーバー（28ツール = 固有27 + プラグイン1、port 8809）
...
985キー/言語。フラットなキーバリューJSON形式。フロントエンド（import）とバックエンド（go:embed）で共用されます。
