...
|---|---|---|---|---|
| **Read専用** | `gkill-read-server.mjs` | 11 | `mcp:gkill-read` / `mcp:gkill-read-http` | 閲覧のみ。データを変更したくない場合 |
| **Write専用** | `gkill-write-server.mjs` | 28 | `mcp:gkill-write` / `mcp:gkill-write-http` | 書き込み中心。Read便利ツール6つ付属 |
| **Read/Write統合** | `gkill-readwrite-server.mjs` | 32 | `mcp:gkill-readwrite` / `mcp:gkill-readwrite-http` | 全機能。迷ったらこれ |
