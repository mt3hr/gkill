...
**スレッドIDは `session_meta.session_id` ではなくファイル名の uuid。** 実データ数十ファイルのうち
23ファイルに `session_id` が無く、あってもサブエージェントでは親のIDが入っているため
（ファイル数より少ない種類しかない）、これをキーにすると親子の Kyou ID が衝突する。
...
- **Codex（ファイル単位の取り込み + スレッド木単位の畳み直し）** — 変わったファイルだけ
  ...
  十数ファイル・百MB超を読み直すことになるため。子が新しく現れたときも親が消えたときも
...
直しようが無かった（2巡目の指摘で分離）。`last_build_error` は直近の構築失敗の
...
- **展開しない。** 中央ディレクトリを読んでエントリを列挙し、読むときにその場で伸長する。
  ...
  展開後 数GB あるうえ、あのキャッシュのキーは `sha1(パス)` だけで中身が変わっても無効化されない
...
- **差分判定は `(CRC32, Size)`。** Takeout は書き出し時刻を全エントリに同じ値で入れるので、
  **エントリの更新時刻は中身が変わっても動かない**（実データで全エントリ（1万件超）が同一値）。
