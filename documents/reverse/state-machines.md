...
| Mi | `ーみ` / `/mi` → タイトル → [ボード名] → [開始予定] → [終了予定] → [期限] |
| MiReKyou | `～～` / `~~` → [ボード名] → [開始予定] → [終了予定] → [期限] → [タグ] → `～～`（タイトル行なし。タグは板名の前にも書ける） |
...
stateDiagram-v2
    ...
    state "Watch App" as watch {
        ...
        アイドル --> 気分選択中: 気分記録<br>(星5個・値は未選択)
        気分選択中 --> 気分確認中: 星の左半分/右半分をタップ<br>(mood 1-10 が決まる)
        気分確認中 --> 気分選択中: キャンセル(選び直し)
        気分確認中 --> 送信中: 確認画面で送信<br>(KFTLテキストへ組み立てて/gkill/submit)

        送信中 --> 結果表示: 結果受信<br>(/gkill/submit_result)
        送信中 --> エラー: タイムアウト/エラー

        送信中 --> 重複確認: DUPLICATE受信<br>(直前に同じ内容を保存済み)
        重複確認 --> 送信中: それでも送信<br>(/gkill/submit_force)
...
タイル（`GkillTileService`）は「記録する」「実行中」「気分記録」への導線を出すだけで、テンプレートは並べない。
3つとも `EXTRA_MODE`（`record` / `playing` / `lantana`）を載せて `MainActivity` を起動する。
