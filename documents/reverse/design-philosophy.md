...
現在の日本語ロケール: `。`=タグ、`ーー`=テキスト、`？`=関連時刻、`、`=区切り 等。
...
| 画面 | 役割 | 主な操作 |
|------|------|---------|
| KFTL (mkfl) | テキストベース記録 | 全データ型の追加、TimeIs の開始/終了 |
| Rykv | ライフログ閲覧 | 検索、閲覧、編集、削除 |
| DNote | 日毎サマリ・集計 | 集計条件設定、グラフ表示 |
| Mi | タスク管理 | タスクの追加/編集/チェック、ボード管理 |
| Nlog | 支出確認 | 支出の追加/編集、金額集計 |
| TimeIs (plaing) | 時間管理 | 稼働中タイマーの表示/終了 |
| Lantana | 気分値記録 | 気分値の入力（ダイアログ） |
| URLog | URL 記録 | ブックマークレットからの記録 |
