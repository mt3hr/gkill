...
- **アカウント機能**: ユーザ ID + パスワード SHA256 でログイン（サーバ側はArgon2idで保存・照合）
...
### パスワードを Argon2id で保存する判断

当初はクライアントが計算した無塩 SHA-256 をそのまま保存し、文字列比較で照合していた。
これは **DB に入っている値がそのままログイン資格情報になる**設計で、実際に
`update_cache` サブコマンドが「保存値を読んで `/api/login` に投げ直す」形で成立していた。
つまり `account.db` を読めた者は全アカウントになりすませた。バックアップ・同期フォルダ・
端末の紛失がそのまま全アカウントの流出になる。

そこで **「DB を読んでもログインできない」状態にすること**を優先し、サーバ側で Argon2id を
かけるようにした。判断の要点は3つある。

- **ワイヤ形式は変えなかった。** クライアントは今も `password_sha256`（64桁hex）を送る。
  これを「資格情報」として扱い、その上に Argon2id をかける。前段の SHA-256 は Argon2id の
  強度を下げない（攻撃者は候補ごとに SHA-256→Argon2id を計算する必要があり、コストは
  Argon2id が支配する）一方、変えれば Web・MCP・Wear OS・gkill_autolog の全実装に波及する。
  副次的に、Argon2id への入力が64桁hex固定になるので長大入力による DoS も塞がる。
  サーバが平文パスワードを見ないという性質も保たれる。
- **既存のハッシュは包み直さず、全員に再設定してもらった。** 包み直しても
  「DB を読めた者がログインできる」状態が続いてしまう。移行の痛みを一度払う側を選んだ。
- **代償として、リセットトークンが唯一の復帰経路になった。** DB から復元できない以上、
  管理者がパスワードを忘れたときに戻る手段が要る。サーバ機上でのみ実行できる CLI
  `reset_password` がその出口で、信頼の根拠は「サーバと同じマシンで設定ディレクトリを
  読み書きできること」に置いている。

### 派生キャッシュを利用者ごとに分ける判断

`/zip_cache/` は認証済みなら誰でも全利用者の展開済み ZIP を読めていた。
同じファイル内の `/files/` は「URL 中の rep 名がその利用者の rep か」を照合して防いでいるが、
**rep 名は利用者間で重複しうる**ため、ZIP キャッシュにはその方式が使えない
（同名 rep を持つ利用者が他人のファイルのハッシュを含む URL を要求すると通ってしまう）。

そこでキャッシュのパスに利用者IDを挟み、**配信の起点をセッション由来のディレクトリに固定**した。
利用者IDは URL に現れないので、他人のディレクトリを指名すること自体ができない。
境界を「名前の照合」ではなく「ディレクトリそのもの」に置いたのがこの判断の要点。
...
サーバ側パーサ（`kftl_factory.go`）とクライアント側パーサ（`src/client/classes/kftl/`、ASCII 定数は `kftl-prefixes.ts` に集約）は、日本語プレフィックスと ASCII プレフィックスの**両方**を常に受理する。
...
| TimeIs | 時間管理 | 稼働中タイマーの表示/終了 ※専用ルート `/playing` |
...
- **MCP Server**: AI エージェントからのアクセス。Read（11ツール、読み取りのみ）に加え、
  Write（28ツール）と ReadWrite（32ツール）があり、追加・更新・削除も行える
