...
9.1. **[scenario.md](scenario.md)** — 利用シナリオ集。チャネル横断（ブラウザ / モバイル / MCP / 共有 / プラグイン）の end-to-end 利用シナリオと、その裏で何が呼ばれどう動くかを UML で詳解します。
...
15. **[api-endpoints.md](api-endpoints.md)** — APIエンドポイント一覧。全91エンドポイントのリファレンスです。
...
| ファイル | 内容 | 主な読者・用途 |
|---|---|---|
| [glossary.md](glossary.md) | ドメイン用語の定義（96項目） | 全員。用語確認時に随時参照 |
...
| [usecase.md](usecase.md) | ユースケース一覧（86件） | 機能仕様の把握、テスト設計 |
...
| [screen-specs.md](screen-specs.md) | 画面仕様・項目定義（334コンポーネント） | UI実装・改修時の参照 |
| [frontend-architecture.md](frontend-architecture.md) | Vue 3フロントエンド設計ガイド | フロントエンド開発者向け |
| [api-endpoints.md](api-endpoints.md) | 全APIエンドポイントのリファレンス（91件） | API利用・実装時の参照 |
...
この資料集が扱うのは **What（現在どうなっているか）** です。
**Why（なぜそうなっているか）**、とくに **採らなかった案とその理由・実測値・撤回した決定** は
[../adr/README.md](../adr/README.md) に置いてあります。

判定基準は「その決定を知らない人が、逆方向の変更を『改善』として提案しうるか」。
しうるなら ADR、しえないならここ（仕様）です。
[design-philosophy.md](design-philosophy.md) が持つのは**覆らない大方針**で、
ADR が持つのは**逆方向の変更が提案されうる個別判断**、という切り分けになっています。
...
graph TD
    ...
    R --> W[plugin-system.md<br/>プラグインシステム]
    W --> F
    W --> U
