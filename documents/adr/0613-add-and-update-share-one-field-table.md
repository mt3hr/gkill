...
| Supersedes | なし |
| Superseded-by | なし |
| Anchors | `src/server/gkill/mcp/write_normalization.go`（`ENTITY_FIELD_SPECS` / `normalizeEntityArgs`）/ `src/server/gkill/mcp/write_handlers.go`（`ENTITY_SUMMARIZERS`）/ `src/server/gkill/mcp/server_base.go`（`handleToolCall`）/ `src/server/gkill/mcp/bootstrap.go` |
...
> `assertUrlWithScheme` は `write_normalization.go` 全体で**追加側の1箇所でしか呼ばれていなかった**。
...
(1) は実害も出していた。実利用レビューが「read と readwrite でスキーマが違う」と
...
- 起動処理は `bootstrap.go` の `startMcpServer(spec)` へ。3ファイルは表の1行だけ持つ
...
- **18本を残したまま `assertUrlWithScheme` を更新側にも足す** — 直るのはこの1件だけ。
  ...
  （各ツールのテストは自分の欄しか見ない）
- **`ENTITY_TARGETS`（`constants.go`）にフィールド表も持たせて表を1つにする** — ADR-0611 と同じ理由で却下。
  `constants.go` は定数の置き場で、正規化関数の関心事を入れると read からも見える。
...
- **3サーバを1ファイルに統合する** — MCP クライアントの設定ファイルがコマンドラインで
  `server_read.go` / `server_readwrite.go` を名指ししており、
...
- **`normalizeEntityArgs` を write-handlers 側へ置く** — 正規化は write-tools のスキーマと対で読むものなので、
  `write_normalization.go` に残す

## Consequences

- `write_normalization.go` が 566行 → 456行。`write_handlers.go` が 297行 →（前回194行）→ 652行
...
- 古スキーマの補足（「ツールスキーマが古い」の印）が read の要約にしか無かった非対称も、
  `appendStaleSchemaNoteToSummary`（`payload.go`）1本にして解消した
- 表に無い kind を書くと `unknown field kind` で即死する。黙って素通しにはしない

## Evidence

- `assertUrlWithScheme` の `write_normalization.go` 内での呼び出しは**1箇所だけ**だった（定義を除く）
