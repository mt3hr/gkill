# ADR-0403: タグ欄付きの追加/編集画面は add_tag が完了してから registered_kyou を emit する
...
このとき **emit の順序が唯一の防御線**になる。局所挿入（→ ADR-0402）は渡された Kyou をそのまま使わず `refresh_kyou` で引き直すので、その時点でサーバにタグが入っていれば `attached_tags` 込みで差し込まれる。
...
- **⊗ を押した既存タグを即座に消す** — 押し間違いを戻せない。保存を押すまで消さず、実削除は `is_deleted=true` の版を足す `update_tag`（→ ADR-0201）。
