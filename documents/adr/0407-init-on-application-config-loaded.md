# ADR-0407: 列ビューの初期化トリガはサイドバーの @inited ではなく ApplicationConfig.is_loaded
...
- **`onMounted` で無条件に初期化する** — `ApplicationConfig` が未ロードのまま既定クエリを作ると、`generate_default_query_for_rykv` が読む `device_struct` / `rep_type_struct` / `tag_struct` / `rykv_default_period` / `hide_tags` が空になり、**既定期間と強制非表示タグが黙って落ちる**（→ ADR-0406）。
