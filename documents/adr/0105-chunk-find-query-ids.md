# ADR-0105: FindQuery.IDs は分割して渡し、検索が失敗したのに GkillError が空のまま return しない
...
- **`Mi` / `MiReKyou` の5射影UNIONを1本にまとめてバインド変数を減らす** — UNION の5本は実仕事をしている（作成/確認/期限/開始/終了の5つの時刻でそれぞれ射影する）。まとめると意味論が変わる。→ ADR-0801 でも「Mi の UNION は実仕事をしている」として同じ結論に達している。
