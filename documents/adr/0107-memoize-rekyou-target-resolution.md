# ADR-0107: ReKyou / MiReKyou のターゲット解決はリクエスト単位でメモ化する
...
- **委譲そのものをやめて、ReKyou 側にターゲットのタイトルを非正規化して持つ** — 追記専用DAO（→ ADR-0201）では、ターゲットが更新されても ReKyou の版は増えないので、非正規化した値が古いまま残る。ターゲットの本文を編集したら参照側の検索結果も変わってほしい、が要件。
