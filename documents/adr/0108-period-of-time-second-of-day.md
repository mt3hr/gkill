# ADR-0108: 時間帯フィルタの秒値は「86400未満は秒オブデイ、以上はepoch」の二重解釈にする
...
- **MCPスキーマ**は最初から「0-86399 の秒オブデイ」を宣言し、値も 0-86399 で検証していた。
  ...
  1年間動いていた**（1年分十数万件の指摘で発覚。窓がずれているだけでフィルタ自体は
...
- 1年分の点検での実測: `period_of_time_start_time_second: 0, end: 21599`（00:00-05:59のつもり）が
  JST 09-14時台の件数（百件余り）と完全一致し、`32400-53999` が JST 18-23時台の実測件数（百件余り）と完全一致した
