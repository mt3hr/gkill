...
- **`gkill_get_idf_file`（base64）も同時に消す** — **実測で否決**（Evidence 参照）。
  ...
  `a510fbc2`（2026-08-19）で入った直後の2日ぶんしかなく、中身はほぼ点検のトラフィックだった。
...
|---|---|---|
| `"path":"/files/` | 0 | **0** |
| `gkill_get_idf_file` | 0 | **3**（`response_bytes` は各数MB） |
| `gkill_get_kyous` | 307 | 308（1回、二十数秒） |
