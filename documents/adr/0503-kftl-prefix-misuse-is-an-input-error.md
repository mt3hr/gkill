# ADR-0503: 引数の無い／引数を同じ行に書いたメモ帳のプレフィックスは、ゼロ値を書かずに行別エラーにする
...
あわせて、ADR-0502 が「打ち間違いを 500 で返さない」と定めたのに、
...
- どちらも `kftl_kmemo.go` / 各 start 行の `ApplyThisLineToRequestMap` で見る。
  **このフェーズはまだ1バイトも書いていない**ので、全行を評価して `errors.Join` で束ね、
  1往復で全部直せる（ADR-0502 の仕組みにそのまま乗る）
...
- **単独プレフィックスを無視して0件にする** — `/expense` などの既存挙動へ揃える案。
  ...
  利用者から見た不可解さは残る。ADR-0502 の趣旨（打ち間違いを見えるようにする）とも合わない
