...
**rep 名は native の git rep と同じくリポジトリごと**（ディレクトリ名）にしたかった。実データでは 88 個の zip に 97 個の `.git`（コミットのあるもので 78 個の名前）が入っている。
...
- **プラグイン1本 = リポジトリ1つにする** — 88 個の zip に 78 個のリポジトリ名。プラグインディレクトリを 78 個作り、配布スクリプトの明示列挙（`Git/scripts` の3ファイル5箇所）に 78 行足すことになる。zip を足すたびに配布が要る。
...
- 実データ: 88 zip（97 の `.git`）→ 78 rep 名 / 3,447 コミット。初回構築は約 60 秒（`get_rep_name` の応答は取り込みが進むにつれ `[]` から増えていく）
