# ADR-0072: 実装されなかった検索条件は消す — 受理して無視するより未知キーとして弾く

| | |
|---|---|
| Status | Accepted |
| Date | 2026-08-24 |
| Sources | ADR-0071 の却下案を1つ覆す |
| Supersedes | なし |
| Superseded-by | なし |
...
ADR-0071 で削除済みの列挙は `IncludeDeletedData` に一本化したが、
...
- **受理して黙って無視する現状のまま残す（ADR-0071 の判断）** — その判断の前提
...
- **`IsDeleted` を「削除済みのみ」の意味で実装する** — `IncludeDeletedData` と意味が重なり、
  1つの旗が層によって反対の意味を持つ状態が固定される（ADR-0071 の議論と同じ）
