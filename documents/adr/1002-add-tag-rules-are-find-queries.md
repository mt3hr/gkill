...
4. **タグ行の ID は画面から付けるときと同じランダムな UUID。判定はタグ名の差分だけなので、画面から消したタグは
   次回付け直される。** 旧 auto_tag は (対象ID, タグ名) から決まる UUIDv5 を使い、消したタグの行が同じ ID で残ることで
   「手で消したタグは付け直さない」を実現していた。要件を確認したところ「付いていなければ付ける」であり、
   その考慮は不要と決まった（2026-09-11、同日中の追加判断）。`CREATE_APP` は `gkill_add_tag`（出所の表示用）。
...
- **旧フラグを互換のために残す** — 「指定したのに効かない」を静かに起こす。呼び出し元は3スクリプトだけで、
  同時に直せる。
- **UUIDv5 の決定的 ID を残す** — 旧 auto_tag 由来。「手で消したタグを復活させない」ために (対象ID, タグ名) から
  ID を計算し、`ERR000056` をスキップとして飲んでいた。要件は「付いていなければ付ける」で、消した記録は
  「付いていない」のだから付け直すのが素直。残すと、名前空間という説明の要る仕組みと `CREATE_APP` の旧名が
  据え置きで残り、消したタグが二度と付かない挙動が要件と食い違ったまま続く。
...
- 付与済み判定を取りこぼしたとき（キャッシュが古いまま実行した等）に同名のタグが2行付くのを防ぐ保険は無い。
  運用スクリプトは直前に `update_cache` を回しているので実用上は起きにくい。旧 auto_tag が付けたタグは
  `CREATE_APP = gkill_auto_tag` のまま残る。
...
- 「消したタグは付いていない扱い」の根拠: `find_filter.go` の `putLatestNotDeleted` がタグフィルタの候補から
  `IsDeleted` の行を外す。付与済み判定はこのフィルタの結果なので、ID をランダムにすれば差分に入る。

## Related tests

- `src/server/gkill/main/common/add_tag_test.go` — ルール JSON の厳格復号、拒否/通過する指定の網羅表、
  クライアントの全キー受理（TS ソース走査）、`rep_types_in_sidebar` の展開、タグ行がランダム UUID であること、HTTP 応答判定（`ERR000056` も失敗）
