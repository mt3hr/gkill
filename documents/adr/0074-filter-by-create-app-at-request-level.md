# ADR-0074: 「どのアプリが書いたか」の絞り込みは FindQuery ではなく MCP リクエストに置く

| | |
|---|---|
| Status | Accepted |
| Date | 2026-08-24 |
| Sources | 2026-08-24 の MCP 再監査（P-39）。ADR-0001 / ADR-0002 が否決した「rep名を SQL へ降ろす」と同じ失敗クラス |
| Supersedes | なし |
| Superseded-by | なし |
...
- **`FindQuery` に足して `GenerateFindSQLCommon` で SQL へ降ろす** — 一見きれいで、
  ...
  **「MCP で作ったリポストだが、リポスト先の記録はブラウザで作った」がエラーも警告も無く消える**。
  ADR-0001 / ADR-0002 が rep 名について否決したのとまったく同じ失敗クラス
