# ADR-0201: Append-Only DAO — ID 列に主キー制約を置かず、更新も削除も INSERT で表現する

| | |
|---|---|
| Status | Accepted |
| Date | 2026-08-21 |
| Sources | `documents/reverse/design-philosophy.md`「3. Append-Only DAO」 / `bb364253`（指摘 H-07） / `.claude/skills/gkill-go-backend/SKILL.md`「型別 GetXxx(id, nil) は最新版を返す」節 |
| Supersedes | なし |
| Superseded-by | なし |
...
削除しても行は残るので、**IDを再利用してはいけない**。かつて auto_tag はタグIDを（対象ID, タグ名）の UUIDv5 にして、この性質を逆手に「再実行で重複させず、**手動で削除したタグは二度と復活しない**」を実現していた（削除された行が同じIDを持つため）。2026-09-11 に後継の `add_tag` はランダムな UUID へ戻した——要件が「付いていなければ付ける」で、消したタグを付け直す必要があったため（[ADR-1002](1002-add-tag-rules-are-find-queries.md)）。
