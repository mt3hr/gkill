# ADR-0063: 同じ対応表を「表」と「直書き」の2形態で持たない
...
- **`/api/get_kyous` を廃して `get_kyous_mcp` へ一本化** — 依頼の文字どおりだが、
  ...
  `get_kyous_mcp` は `limit` 1..1000 の厳密上限が契約（ADR-0053）なので、寄せると両方壊れる。
...
- **`GetAllTagNames` と `…IncludingDeletedTargets` を統合** — 意図的な対（ADR-0073）。
...
- `wrapNoAuth` の MCP 2本にも `readAuthBody` の 32MB 上限が掛かるようになった
  （アップロード系のための免除が MCP にも及んでいた）
