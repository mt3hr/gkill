...
- 2026-09-15 追記: 「日時として読めないその他の行は未設定のまま」の部分は
  [ADR-0508](0508-kftl-blank-records-are-input-errors.md) で改め、空でないのに読めない行も入力エラーになった
  （`、` で6行を埋めずに次の記録へ移ると後ろの本文が欄に食われていた）。`？` の禁止はそのまま
