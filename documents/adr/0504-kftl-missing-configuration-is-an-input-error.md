# ADR-0504: KFTL の実行フェーズの失敗も、設定不足なら行別の入力エラーにする
...
が返る（[ADR-0502](0502-kftl-errors-are-per-line.md) / [ADR-0503](0503-kftl-prefix-misuse-is-an-input-error.md)）。
落差の理由は2つ。
...
ADR-0502 は「実行フェーズは既定でサーバ障害」と決めており、それ自体は正しい。
...
（[ADR-0707](0707-redact-environment-specific-strings.md)）。
...
- **実行フェーズ全体を入力エラー扱いにする** — DB の書き込み失敗や rep の I/O エラーまで
  ...
  ADR-0502 の「既定はサーバ障害」を崩さず、**名指しした分だけ**入力エラーへ倒す
