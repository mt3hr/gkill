...
- **数千コミット、ハッシュの重複除去で1割弱減る**。同じリポジトリを別の時期に固めた zip が複数組あり、改名前後で履歴を共有する組もある
- **数百コミットは稼働中のリポジトリにも同じハッシュがある**
...
- `rep` タグ百件余りのうち `.git/HEAD` 入りが 8 割弱。`.git` の位置は `name/.git/`（大半）・ルート直下 `.git/`・1 zip に複数・入れ子
- loose のみ / pack のみ / 両方が混在。`git init` 直後（コミット 0 件）が十数個
- 数千コミット → 1割弱が重複（同じリポジトリの zip と改名前後の組）。数百件は最近の zip を足す前の時点で稼働中 rep にもあった
- 初回構築 約 1 分（数十 zip・数千コミット・go-git の行数集計込み。最大の zip は数十MB）。2回目以降は指紋（sha256 of (エントリ名, CRC32, サイズ)）の比較だけ
