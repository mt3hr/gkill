# ADR-0208: gitリポジトリ判定は PlainOpen のエラー型ではなく os.Stat で行う
...
- **`PlainOpen` のエラーを一律 `ErrNotGitRepository` に丸める** — 権限エラーや壊れたリポジトリまでスキップに化けて、**gitのコミットログが1件も出ないことに気付けなくなる**。`TestNewGitRepSuccessForGitRepository` がその意図で置いてある。
  ...
  > この却下が禁じているのは**黙る**ことであって、切り離すこと自体ではない。
  > 読めない rep を「利用者へ必ず返したうえで」切り離す形は [ADR-0216](0216-detach-a-broken-rep-but-never-silently.md) で採用した。
