// gkill_get_mcp_help が返す詳しい案内。3サーバ共通。
//
...
// tools/list のバイト量は tool-schema-budget.json が固定し、増加は予算テストが止める。
//
// 本文は英語（説明文と同じ）。ここに書けるツール名は、その topic を読むサーバに載っていなくてもよい
// （本文は「今これを呼べ」ではなく参照情報。tool-handlers.test.mjs の走査対象は説明文だけ）が、
// 実在しないツール名は help-topics.test.mjs が落とす。
...
const PAGINATION_TOPIC =
  "Pagination: pass next_cursor from the previous response verbatim as cursor — do not construct or edit it " +
...
const MI_TOPIC =
  "Mi (task) entries surface under FIVE projections, one per timestamp: mi_create (create_time), mi_check " +
...
const DATA_TYPES_TOPIC =
  "data_type has TWO vocabularies. Search results and add_* / update_* responses carry PROJECTION names: kmemo, " +
...
const IDF_TOPIC =
  "IDF entries (data_type 'idf') are files: images, videos, audio, zip/cbz and others. The payload carries " +
...
const KFTL_TOPIC =
  "KFTL is gkill's line-based text format that creates multiple records from a single text block, submitted " +
...
// topic 名 → {title, text}。index は buildHelpIndexText が組み立てる。
export const HELP_TOPICS = Object.freeze({
  search: Object.freeze({
    title: "gkill_get_kyous: query semantics, response fields, warnings, payload shapes",
    text: SEARCH_TOPIC,
  }),
  pagination: Object.freeze({
    title: "Paging (cursor / remaining_count), limit is not speed, count_only and group_by",
    text: PAGINATION_TOPIC,
  }),
  mi: Object.freeze({
    title: "Mi tasks: the five projections, for_mi, include_*_mi, mi_sort_type",
    text: MI_TOPIC,
  }),
  data_types: Object.freeze({
    title: "data_type vocabularies, the data_types filter, num_min / num_max, idf_kinds, create_apps",
    text: DATA_TYPES_TOPIC,
  }),
  plugin: Object.freeze({
    title: "Plugin entries and include_plugin_content (budget, format, content_status)",
    text: PLUGIN_TOPIC,
  }),
  idf: Object.freeze({
    title: "Files and images: file_path / gkill_get_idf_file / file_url, thumbnails, index freshness",
    text: IDF_TOPIC,
  }),
  deleted: Object.freeze({
    title: "Deleted entries and version history: include_deleted_data, gkill_get_kyou_history, restore",
    text: DELETED_TOPIC,
  }),
  rep: Object.freeze({
    title: "rep_types vs rep names, gkill_get_rep_infos, attached_data_reps, use_to_write",
    text: REP_TOPIC,
  }),
  kftl: Object.freeze({
    title: "KFTL text format for gkill_submit_kftl: every prefix, ~~ and ?? blocks, failures, provenance",
    text: KFTL_TOPIC,
  }),
});

// index は topic を省略したときの応答。topic 名は inputSchema の enum にもなる。
export const HELP_INDEX_TOPIC = "index";

export const HELP_TOPIC_NAMES = Object.freeze([HELP_INDEX_TOPIC, ...Object.keys(HELP_TOPICS)]);

// listHelpTopics は {topic, title} の一覧（index の本体）。
export function listHelpTopics() {
  return Object.entries(HELP_TOPICS).map(([topic, entry]) => ({ topic, title: entry.title }));
}
...
function buildHelpIndexText() {
  ...
  for (const { topic, title } of listHelpTopics()) {
    lines.push(`- ${topic}: ${title}`);
  }
  lines.push("Call gkill_get_mcp_help with topic:<name> to read one. gkill_status reports the account and the " +
...
export function buildHelpPayload(topic) {
  const name = topic === undefined || topic === null || topic === "" ? HELP_INDEX_TOPIC : topic;
  if (name === HELP_INDEX_TOPIC) {
    return {
      topic: HELP_INDEX_TOPIC,
      title: "Help index",
      text: buildHelpIndexText(),
      topics: listHelpTopics(),
    };
  }
  const entry = HELP_TOPICS[name];
  if (!entry) {
    return {
      topic: HELP_INDEX_TOPIC,
      title: "Help index",
  ...
  return { topic: name, title: entry.title, text: entry.text };
}
