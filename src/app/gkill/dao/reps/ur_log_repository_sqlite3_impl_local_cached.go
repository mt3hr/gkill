...
func (u *urlogRepositorySQLite3ImplLocalCached) UpdateCache(ctx context.Context) error {
	...
	localCacheDBFileName := filepath.Join(os.ExpandEnv(gkill_options.CacheDir), "local_cache_rep", strings.Replace(u.originalDBFileName, ":", "", -1))
	localCacheDBParentDirName, _ := filepath.Split(localCacheDBFileName)
	...
	if updateCache {
		...
		if err != nil {
			err = fmt.Errorf("error at open file %s: %w", u.originalDBFileName)
		}
		defer originalDBFile.Close()
		cacheDBFile, err := os.Create(localCacheDBFileName)
		if err != nil {
