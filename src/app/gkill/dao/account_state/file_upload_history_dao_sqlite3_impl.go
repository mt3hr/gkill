...
const CURRENT_SCHEMA_VERSION_FILE_UPLOAD_HISTORY_DAO = "1.0.0"
...
func NewFileUploadHistoryDAOSQLite3Impl(ctx context.Context, filename string) (FileUploadHistoryDAO, error) {
	...
	if isOld, oldVerDAO, err := checkAndResolveDataSchemaFileUploadHistoryDAO(ctx, db); err != nil {
		return nil, err
	} else if isOld {
		if oldVerDAO != nil {
...
func checkAndResolveDataSchemaFileUploadHistoryDAO(ctx context.Context, db *sql.DB) (isOld bool, oldVerDAO FileUploadHistoryDAO, err error) {
	schemaVersionKey := "SCHEMA_VERSION_FILE_UPLOAD_HISTORY"
	currentSchemaVersion := CURRENT_SCHEMA_VERSION_FILE_UPLOAD_HISTORY_DAO
