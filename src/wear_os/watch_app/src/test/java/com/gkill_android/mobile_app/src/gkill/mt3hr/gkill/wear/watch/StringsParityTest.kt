 ...
 *
 * phone_companion にも同型のテストがある（モジュールをまたぐ共有はしない。verify_docs の件数集計が
 * モジュール単位のため）。直すときは両方へ。こちらは server_locale_name を持たない
 * （locale_name はスマホが決める）ので、その検査だけ無い。
 */
class StringsParityTest {
    ...
    private val resDir: File by lazy {
        val candidates = listOf("src/main/res", "watch_app/src/main/res")
    ...
    fun nonTranslatableKeys_areNotRepeatedInLocales() {
        ...
        assertTrue("expected tile_label to be translatable=false", "tile_label" in fixed)
        for ((locale, strings) in localeStrings) {
