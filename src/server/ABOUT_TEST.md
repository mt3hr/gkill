...
Go バックエンドのテスト。905テスト関数、125テストファイル、30パッケージで構成される。API ハンドラ統合テスト、DAO 層テスト、プラグインのサブプロセス管理テスト、プラグインSDKテスト、KFTL パーサテスト、CLI テストを網羅する。
...
| リポジトリ | 30 | `reps/` 直下28（13データ型の SQLite3 実装 + キャッシュ + 一時 + プラグイン + デッドロック/ネスト並列回帰）+ `reps/cache/`・`reps/rep_cache_updater/` の2 |
