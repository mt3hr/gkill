...
Go バックエンドのテスト。1007テスト関数、149テストファイル、31パッケージで構成される。API ハンドラ統合テスト、DAO 層テスト、プラグインのサブプロセス管理テスト、プラグインSDKテスト、KFTL パーサテスト、CLI テストを網羅する。
...
| リポジトリ | 52 | `reps/` 直下49（13データ型の SQLite3 実装 + キャッシュ + 一時 + プラグイン + デッドロック/ネスト並列回帰 + gitキャッシュUNIQUE + GPS集約dedup）+ `reps/cache/`・`reps/rep_cache_updater/` の2 |
...
**合計 149 ファイル**（上表の合計。`node src/tools/verify_docs.mjs --list` が出す `goTestFiles` と一致する。
