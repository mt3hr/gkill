...
func TestExportIDOf_SplitPartsShareOneExport(t *testing.T) {
	dir := filepath.FromSlash("/tmp/GoogleTakeout_TestPC_20260811")
...
func TestOpenSources_ExportsAreSortedNewestFirst(t *testing.T) {
	root := t.TempDir()
	oldDir := filepath.Join(root, "GoogleTakeout_TestPC_20260101")
	newDir := filepath.Join(root, "GoogleTakeout_TestPC_20260811")
