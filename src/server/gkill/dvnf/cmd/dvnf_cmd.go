...
func init() {
	...
	DVNFCmd.PersistentPreRun = func(cmd *cobra.Command, args []string) {
		...
		if err != nil {
			...
			slog.Log(ctx, gkill_log.Debug, "error at get serverConfig", "error", fmt.Sprintf("%q", err))
			return
		}
		...
		if currentServerConfig == nil {
			if rootOpt.device == "" {
				...
				slog.Log(ctx, gkill_log.Debug, "this device has no enabled server config. specify --", "error", fmt.Sprintf("%q", err))
				return
			}
