...
├── mcp.go                 # mcp サブコマンド（gkill_server mcp。稼働中サーバの HTTP クライアント。実装は gkill/mcp）
...
- `mcp` — MCP サーバ（`--kind read|write|readwrite`、`--transport`、`--config`。子コマンド `schema-budget`。定義は `mcp.go`、実装は `gkill/mcp` パッケージ）
