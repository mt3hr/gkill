...
func init() {
	...
	AppCmd.PersistentFlags().StringVar(&gkill_log.LogLevelFromCmd, "log", gkill_log.LogLevelFromCmd, "log level [none, error, warn, info, access, debug, trace, trace_sql]")
	AppCmd.PersistentFlags().Int64Var(&gkill_options.LogRotateMaxBytes, "log_rotate_max_bytes", gkill_options.LogRotateMaxBytes, "max bytes per log file. 0 or less disables rotation")
	AppCmd.PersistentFlags().IntVar(&gkill_options.LogRotateKeep, "log_rotate_keep", gkill_options.LogRotateKeep, "number of rotated log files to keep")
