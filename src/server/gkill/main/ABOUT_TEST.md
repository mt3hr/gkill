...
| ファイル | テスト内容 |
|---------|-----------|
| `gkill/main_test.go` | デスクトップアプリ（go-astilectron）エントリポイント。登録するサブコマンドの集合を固定し、サーバ版だけの `idf` / `mcp` が無いこと |
| `gkill_server/main_test.go` | HTTP サーバエントリポイント。登録するサブコマンドの集合（`mcp` / `generate_plugin_cache` を含む12本）を固定する。「1つでもあれば緑」ではなく集合で見るので、`AddCommand` を1行落としても気付ける |
...
- **エントリポイント**: cobra コマンド登録。登録するサブコマンドの集合（gkill_server は `idf` / `mcp` を含む12本、gkill は含まない10本）を固定する
