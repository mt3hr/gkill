...
func (g *GkillServerAPI) HandleOpenDirectory(w http.ResponseWriter, r *http.Request) {
	...
	defer func() {
		...
		if err != nil {
			...
			slog.Log(r.Context(), gkill_log.Debug, "error at parse open directory response to json", "error", fmt.Sprintf("%q", err))
	...
	if err != nil {
		...
		slog.Log(r.Context(), gkill_log.Debug, "error at parse open directory request to json", "error", fmt.Sprintf("%q", err))
		gkillError := &message.GkillError{
