...
func (g *GkillServerAPI) HandleGetURLog(w http.ResponseWriter, r *http.Request) {
	...
	defer func() {
		...
		if err != nil {
			...
			slog.Log(r.Context(), gkill_log.Debug, "error at parse get urlog response to json", "error", fmt.Sprintf("%q", err))
			gkillError := &message.GkillError{
				ErrorCode:    message.InvalidGetURLogResponseDataError,
				ErrorMessage: api.GetLocalizer(request.LocaleName).MustLocalizeMessage(&i18n.Message{ID: "FAILED_GET_URLOG_MESSAGE"}),
				Cause:        err,
			}
	...
	if err != nil {
		...
		slog.Log(r.Context(), gkill_log.Debug, "error at parse get urlog request to json", "error", fmt.Sprintf("%q", err))
		gkillError := &message.GkillError{
	...
	urlogHistories, gkillErrors, err := g.UsecaseCtx.GetURLogHistories(r.Context(), repositories, userID, device, request.LocaleName, request.ID, request.RepName)
	if err != nil {
		slog.Log(r.Context(), gkill_log.Debug, "error at get ur log histories", "error", fmt.Sprintf("%q", err))
		...
		gkillErrors = message.EnsureNotEmpty(gkillErrors, message.GetURLogError,
			api.GetLocalizer(request.LocaleName).MustLocalizeMessage(&i18n.Message{ID: "FAILED_GET_URLOG_MESSAGE"}), err)
	...
	response.Messages = append(response.Messages, &message.GkillMessage{
		...
		Message:     api.GetLocalizer(request.LocaleName).MustLocalizeMessage(&i18n.Message{ID: "SUCCESS_GET_URLOG_MESSAGE"}),
	})
}
