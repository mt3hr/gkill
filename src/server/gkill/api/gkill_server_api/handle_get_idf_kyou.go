...
func (g *GkillServerAPI) HandleGetIDFKyou(w http.ResponseWriter, r *http.Request) {
	...
	defer func() {
		...
		if err != nil {
			...
			slog.Log(r.Context(), gkill_log.Debug, "error at parse get idfKyou response to json", "error", fmt.Sprintf("%q", err))
	...
	if err != nil {
		...
		slog.Log(r.Context(), gkill_log.Debug, "error at parse get idfKyou request to json", "error", fmt.Sprintf("%q", err))
		gkillError := &message.GkillError{
	...
	if err != nil {
		slog.Log(r.Context(), gkill_log.Debug, "error at get idf kyou histories", "error", fmt.Sprintf("%q", err))
		...
		gkillErrors = message.EnsureNotEmpty(gkillErrors, message.GetIDFKyouError,
			api.GetLocalizer(request.LocaleName).MustLocalizeMessage(&i18n.Message{ID: "FAILED_GET_IDFKYOU_MESSAGE"}), err)
