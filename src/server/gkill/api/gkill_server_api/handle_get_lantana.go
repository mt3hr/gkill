...
// HandleGetLantana は、指定IDのLantana（気分値）の更新履歴を返します。
//
// POST /api/get_lantana（wrapAuthRepos）
// req_res.GetLantanaRequest / req_res.GetLantanaResponse
//
// RepNameを指定するとそのrepだけを、nilなら全Lantana repを横断して集めます。
...
func (g *GkillServerAPI) HandleGetLantana(w http.ResponseWriter, r *http.Request) {
	...
	defer func() {
		...
		if err != nil {
			...
			slog.Log(r.Context(), gkill_log.Debug, "error at parse get lantana response to json", "error", fmt.Sprintf("%q", err))
	...
	if err != nil {
		...
		slog.Log(r.Context(), gkill_log.Debug, "error at parse get lantana request to json", "error", fmt.Sprintf("%q", err))
		gkillError := &message.GkillError{
	...
	if err != nil {
		slog.Log(r.Context(), gkill_log.Debug, "error at get lantana histories", "error", fmt.Sprintf("%q", err))
		...
		gkillErrors = message.EnsureNotEmpty(gkillErrors, message.GetLantanaError,
			api.GetLocalizer(request.LocaleName).MustLocalizeMessage(&i18n.Message{ID: "FAILED_GET_LANTANA_MESSAGE"}), err)
