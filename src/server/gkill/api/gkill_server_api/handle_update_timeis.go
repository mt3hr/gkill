...
// HandleUpdateTimeis は時間計測のKyouを更新します。計測の終了もこの経路で、
// 終了時刻を入れた版を書き込むことで行います。
//
// POST /api/update_timeis（wrapAuthRepos）
// req_res.UpdateTimeisRequest / req_res.UpdateTimeisResponse
//
// 更新は追記です（同一IDに新しいUPDATE_TIME版を足す）。TXIDが非nilならtempリポジトリに積むだけで、
// commit_txするまで実リポジトリには反映されません。対象IDが存在しない場合はerrorsに載せて返します。
// WantResponseKyouがtrueのときだけTimeIsとKyouを読み直して返します
...
func (g *GkillServerAPI) HandleUpdateTimeis(w http.ResponseWriter, r *http.Request) {
	...
	defer func() {
		...
		if err != nil {
			...
			slog.Log(r.Context(), gkill_log.Debug, "error at parse update timeis response to json", "error", fmt.Sprintf("%q", err))
	...
	if err != nil {
		...
		slog.Log(r.Context(), gkill_log.Debug, "error at parse update timeis request to json", "error", fmt.Sprintf("%q", err))
		gkillError := &message.GkillError{
	...
	if err != nil {
		slog.Log(r.Context(), gkill_log.Debug, "error at update time is", "error", fmt.Sprintf("%q", err))
		gkillError := &message.GkillError{
	...
	if request.WantResponseKyou {
		...
		if err != nil {
			...
			slog.Log(r.Context(), gkill_log.Debug, "error at get timeis user id", "error", fmt.Sprintf("%q", err))
			gkillError := &message.GkillError{
