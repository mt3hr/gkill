...
func (g *GkillServerAPI) HandleGetAllRepNames(w http.ResponseWriter, r *http.Request) {
	...
	defer func() {
		...
		if err != nil {
			...
			slog.Log(r.Context(), gkill_log.Debug, "error at parse get kyous response to json", "error", fmt.Sprintf("%q", err))
	...
	if err != nil {
		...
		slog.Log(r.Context(), gkill_log.Debug, "error at parse get kyous request to json", "error", fmt.Sprintf("%q", err))
		gkillError := &message.GkillError{
	...
	if err != nil {
		slog.Log(r.Context(), gkill_log.Debug, "error at get all rep names", "error", fmt.Sprintf("%q", err))
		...
		gkillErrors = message.EnsureNotEmpty(gkillErrors, message.GetAllRepNamesError,
			api.GetLocalizer(request.LocaleName).MustLocalizeMessage(&i18n.Message{ID: "FAILED_GET_ALL_REP_NAMES_MESSAGE"}), err)
