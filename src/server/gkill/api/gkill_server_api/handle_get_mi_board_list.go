...
func (g *GkillServerAPI) HandleGetMiBoardList(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		gkillErrors = message.EnsureNotEmpty(gkillErrors, message.GetMiBoardNamesError,
			api.GetLocalizer(request.LocaleName).MustLocalizeMessage(&i18n.Message{ID: "FAILED_GET_MIBOARD_NAMES_MESSAGE"}), err)
	...
	response.Messages = append(response.Messages, &message.GkillMessage{
		...
		Message:     api.GetLocalizer(request.LocaleName).MustLocalizeMessage(&i18n.Message{ID: "SUCCESS_GET_MIBOARD_NAMES_MESSAGE"}),
	})
}
