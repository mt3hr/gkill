...
func (g *GkillServerAPI) HandleUpdateRekyou(w http.ResponseWriter, r *http.Request) {
	...
	defer func() {
		...
		if err != nil {
			...
			slog.Log(r.Context(), gkill_log.Debug, "error at parse update rekyou response to json", "error", fmt.Sprintf("%q", err))
			gkillError := &message.GkillError{
				ErrorCode:    message.InvalidUpdateReKyouResponseDataError,
				ErrorMessage: api.GetLocalizer(request.LocaleName).MustLocalizeMessage(&i18n.Message{ID: "FAILED_UPDATE_REKYOU_MESSAGE"}),
				Cause:        err,
			}
	...
	if err != nil {
		...
		slog.Log(r.Context(), gkill_log.Debug, "error at parse update rekyou request to json", "error", fmt.Sprintf("%q", err))
		gkillError := &message.GkillError{
	...
	gkillErrors, err := g.UsecaseCtx.UpdateReKyou(r.Context(), repositories, userID, device, request.LocaleName, request.ReKyou, request.TXID)
	if err != nil {
		slog.Log(r.Context(), gkill_log.Debug, "error at update re kyou", "error", fmt.Sprintf("%q", err))
		gkillError := &message.GkillError{
	...
	if request.WantResponseKyou {
		...
		if err != nil {
			...
			slog.Log(r.Context(), gkill_log.Debug, "error at get rekyou user id", "error", fmt.Sprintf("%q", err))
			gkillError := &message.GkillError{
				ErrorCode:    message.GetReKyouError,
				ErrorMessage: api.GetLocalizer(request.LocaleName).MustLocalizeMessage(&i18n.Message{ID: "FAILED_UPDATE_REKYOU_UPDATED_GET_MESSAGE"}),
				Cause:        err,
			}
			response.Errors = append(response.Errors, gkillError)
			return
	...
	response.Messages = append(response.Messages, &message.GkillMessage{
		...
		Message:     api.GetLocalizer(request.LocaleName).MustLocalizeMessage(&i18n.Message{ID: "SUCCESS_UPDATE_REKYOU_MESSAGE"}),
	})
}
