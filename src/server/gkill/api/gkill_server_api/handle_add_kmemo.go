...
// HandleAddKmemo はメモ（Kmemo）を1件追加します。
//
// POST /api/add_kmemo（wrapAuthRepos）
// req_res.AddKmemoRequest / req_res.AddKmemoResponse
//
// TXIDがnilなら書き込み用リポジトリへ確定登録し、非nilならそのトランザクションの
// 一時リポジトリへ積む（commit_txするまで検索には出ない）。
// 同じIDのKmemoが既にある場合は追加せず、AlreadyExistKmemoErrorをerrorsに積んで返す。
// WantResponseKyouがtrueのときだけ、登録後にリポジトリから読み直してAddedKmemoとAddedKyouを返す
...
func (g *GkillServerAPI) HandleAddKmemo(w http.ResponseWriter, r *http.Request) {
	...
	defer func() {
		...
		if err != nil {
			...
			slog.Log(r.Context(), gkill_log.Debug, "error at parse add kmemo response to json", "error", fmt.Sprintf("%q", err))
	...
	if err != nil {
		...
		slog.Log(r.Context(), gkill_log.Debug, "error at parse add kmemo request to json", "error", fmt.Sprintf("%q", err))
		gkillError := &message.GkillError{
	...
	gkillErrors, err := g.UsecaseCtx.AddKmemo(r.Context(), repositories, userID, device, request.LocaleName, request.Kmemo, request.TXID)
	if err != nil {
		slog.Log(r.Context(), gkill_log.Debug, "error at add kmemo", "error", fmt.Sprintf("%q", err))
		gkillError := &message.GkillError{
	...
	if request.WantResponseKyou {
		...
		if err != nil {
			...
			slog.Log(r.Context(), gkill_log.Debug, "error at get kmemo user id", "error", fmt.Sprintf("%q", err))
			gkillError := &message.GkillError{
		...
		if err != nil {
			...
			slog.Log(r.Context(), gkill_log.Debug, "error at get kyou user id", "error", fmt.Sprintf("%q", err))
			gkillError := &message.GkillError{
				ErrorCode:    message.GetKmemoError,
				ErrorMessage: api.GetLocalizer(request.LocaleName).MustLocalizeMessage(&i18n.Message{ID: "FAILED_ADD_KMEMO_ADDED_GET_MESSAGE"}),
				Cause:        err,
	...
	response.Messages = append(response.Messages, &message.GkillMessage{
		...
		Message:     api.GetLocalizer(request.LocaleName).MustLocalizeMessage(&i18n.Message{ID: "SUCCESS_ADD_KMEMO_MESSAGE"}),
	})
}
