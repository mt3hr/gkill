...
├── gkill_server_api/            # HTTP ハンドラ（149ファイル）
...
handle_*.go は107ファイル（実装91 + テスト16）で、1ハンドラ1ファイルとして分割されている。
...
| `error_codes.go` | エラーコード定数（378定数、ERR000001〜ERR000419・欠番41。うち37は存在しないエンドポイントのコードを 2026-09-14 に削除したもの。ADR-0709） |
