...
// 書き込みツールの定義。write / readwrite の2サーバが共有する（旧 write-tools.mjs）。
//
...
func addedOverlapDesc() string {
	return "The two overlap on purpose: added_kyou is the row shape a timeline shows (id, data_type, related_time, is_image/is_video), added_* is the entity with the fields you just wrote. Use added_*.id as the id everywhere — they are the same id. "
}

// targetsProperty は gkill_delete_kyou / gkill_restore_kyou の targets 引数（verb は delete / restore）。
func targetsProperty(verb string) *jsonobj.Object {
	return jsonobj.Obj(
		"type", "array",
		"items", jsonobj.Obj(
			"type", "object",
			"properties", jsonobj.Obj(
				"id", jsonobj.Obj("type", "string"),
				"data_type", jsonobj.Obj("type", "string", "enum", stringsToAny(EntityAndProjectionDataTypeValues)),
			),
			"required", jsonobj.Strings("id", "data_type"),
			"additionalProperties", false,
		),
		"description",
		"Batch form: "+verb+" several entries in one call, instead of id + data_type. "+
...
var WriteTools = []*jsonobj.Object{
	tool(
		...
		schema(jsonobj.Obj(
			"content", jsonobj.Obj("type", "string", "description", "Memo text content. Supports any free-form text including multi-line."),
			"related_time", jsonobj.Obj("type", "string", "description", "When this memo relates to (not when it was created — that is auto-set). "+ISODateTimeDesc+" or "+DateOnlyDesc+". Defaults to now."),
	...
	tool(
		...
		schema(jsonobj.Obj(
			"url", jsonobj.Obj("type", "string", "description", "Full URL to bookmark (e.g., https://example.com/article)."),
			"title", jsonobj.Obj("type", "string", "description", "Human-readable title for the bookmark. Optional — if omitted and fetch_metadata is true (the default), the server fetches the page and fills the title from its <title> tag (see the outbound-fetch NOTE above); with fetch_metadata:false, or when that fetch fails, the bookmark is stored with an empty title."),
			"related_time", jsonobj.Obj("type", "string", "description", "When this bookmark relates to. "+ISODateTimeDesc+" or "+DateOnlyDesc+". Defaults to now."),
			"fetch_metadata", jsonobj.Obj(
				"type", "boolean",
				"description",
	...
	tool(
		...
		schema(jsonobj.Obj(
			"title", jsonobj.Obj("type", "string", "description", "Description of the expense/income (e.g., \"lunch\", \"train ticket\", \"freelance payment\")."),
			"amount", jsonobj.Obj("type", "integer", "description", "Monetary amount (integer only, e.g. -1500 for expense, 200 for income). Must be a valid integer — empty or non-integer values are rejected by the server."),
			"shop", jsonobj.Obj("type", "string", "description", "Shop, store, or source name (e.g., \"Starbucks\", \"Amazon\"). Optional."),
			"related_time", jsonobj.Obj("type", "string", "description", "When the transaction occurred. "+ISODateTimeDesc+" or "+DateOnlyDesc+". Defaults to now."),
			"locale_name", jsonobj.Obj("type", "string", "description", localeNameDesc),
		), []string{"title", "amount"}),
	),
	tool(
		...
		schema(jsonobj.Obj(
			"mood", jsonobj.Obj("type", "integer", "description", "Mood level: 0 (lowest) to 10 (highest). Must be an integer.", "minimum", 0, "maximum", 10),
			"related_time", jsonobj.Obj("type", "string", "description", "When this mood assessment relates to. "+ISODateTimeDesc+" or "+DateOnlyDesc+". Defaults to now."),
			"locale_name", jsonobj.Obj("type", "string", "description", localeNameDesc),
		), []string{"mood"}),
	),
	tool(
		...
		schema(jsonobj.Obj(
			"title", jsonobj.Obj("type", "string", "description", "Activity title/label (e.g., \"work\", \"meeting\", \"sleep\", \"exercise\")."),
			"start_time", jsonobj.Obj("type", "string", "description", "When the activity started. "+ISODateTimeDesc+" or "+DateOnlyDesc+". Defaults to now."),
			"end_time", jsonobj.Obj("type", "string", "description", "When the activity ended. "+ISODateTimeDesc+" or "+DateOnlyDesc+". Omit for an ongoing interval that hasn't ended yet."),
			"locale_name", jsonobj.Obj("type", "string", "description", localeNameDesc),
		), []string{"title"}),
	),
	tool(
		...
		schema(jsonobj.Obj(
			"title", jsonobj.Obj("type", "string", "description", "Task title/description. Be concise but descriptive."),
			"board_name", jsonobj.Obj("type", "string", "description", "Board name to place the task on. Use gkill_get_mi_board_list to discover existing names. Any string is accepted — a non-existent name creates a new board (unless allow_create_board is false). If omitted, the account's default board is used."),
			"allow_create_board", jsonobj.Obj(
				"type", "boolean",
				"description",
			...
			"is_checked", jsonobj.Obj("type", "boolean", "description", "Whether the task is already completed. Default: false. Set to true to create a pre-completed task (e.g., logging past work)."),
			"limit_time", jsonobj.Obj("type", "string", "description", "Deadline for the task. "+ISODateTimeDesc+" or "+DateOnlyDesc+" — a date-only value expands to the END of that day (23:59:59 local), so \"2026-08-25\" means \"due by the end of the 25th\". Optional."),
			"estimate_start_time", jsonobj.Obj("type", "string", "description", "Estimated start time for scheduling. "+ISODateTimeDesc+" or "+DateOnlyDesc+" — a date-only value expands to the START of that day (00:00:00 local). Optional."),
			"estimate_end_time", jsonobj.Obj("type", "string", "description", "Estimated end time for scheduling. "+ISODateTimeDesc+" or "+DateOnlyDesc+" — a date-only value expands to the END of that day (23:59:59 local). Optional."),
			"locale_name", jsonobj.Obj("type", "string", "description", localeNameDesc),
		), []string{"title"}),
	),
	tool(
		...
		schema(jsonobj.Obj(
			"title", jsonobj.Obj("type", "string", "description", "What is being measured (e.g., \"steps\", \"weight\", \"temperature\", \"study hours\")."),
			"num_value", jsonobj.Obj("type", "number", "description", "Numeric measurement value. Integer or decimal (e.g., 10000, 72.5, -3)."),
			"related_time", jsonobj.Obj("type", "string", "description", "When this measurement was taken. "+ISODateTimeDesc+" or "+DateOnlyDesc+". Defaults to now."),
			"locale_name", jsonobj.Obj("type", "string", "description", localeNameDesc),
		), []string{"title", "num_value"}),
	),
	tool(
		"gkill_add_tag",
		...
		schema(jsonobj.Obj(
			"tag", jsonobj.Obj("type", "string", "description", "Tag name string. Free-form text (e.g., \"work\", \"personal\", \"important\", \"recipe\")."),
			"target_id", jsonobj.Obj("type", "string", "description", "ID of the existing kyou entry to tag. Obtain from the response of gkill_add_kmemo, gkill_add_mi, or any other gkill_add_* tool."),
			"locale_name", jsonobj.Obj("type", "string", "description", localeNameDesc),
		), []string{"tag", "target_id"}),
	),
	tool(
		"gkill_add_text",
		...
		schema(jsonobj.Obj(
			"text", jsonobj.Obj("type", "string", "description", "Text annotation content. Supports free-form text including multi-line."),
			"target_id", jsonobj.Obj("type", "string", "description", "ID of the existing kyou entry to annotate. Obtain from the response of any gkill_add_* tool."),
			"locale_name", jsonobj.Obj("type", "string", "description", localeNameDesc),
		), []string{"text", "target_id"}),
	),
	tool(
		"gkill_submit_kftl",
	...
	tool(
		...
		schema(jsonobj.Obj(
			"id", jsonobj.Obj("type", "string", "description", "ID of the entry to soft-delete. Obtain from gkill_add_* responses, or from gkill_get_kyous on the read / readwrite servers."),
			"targets", targetsProperty("delete"),
			"data_type", jsonobj.Obj(
				"type", "string",
				"description",
				"Data type of the entry to delete. Must match the actual type of the entry. "+projectionVocabularyDesc,
	...
	tool(
		...
		schema(jsonobj.Obj(
			"id", jsonobj.Obj("type", "string", "description", "ID of the kmemo to update. Obtain from gkill_add_kmemo response (added_kmemo.id) or gkill_get_kyous."),
			"content", jsonobj.Obj("type", "string", "description", "New memo text content."),
			"related_time", jsonobj.Obj("type", "string", "description", "New related time (when the memo relates to). "+ISODateTimeDesc+" or "+DateOnlyDesc+". Omit to keep unchanged."),
			"locale_name", jsonobj.Obj("type", "string", "description", localeNameDesc),
		), []string{"id"}),
	),
	tool(
		...
		schema(jsonobj.Obj(
			"id", jsonobj.Obj("type", "string", "description", "ID of the urlog to update. Obtain from gkill_add_urlog response or gkill_get_kyous."),
			"url", jsonobj.Obj("type", "string", "description", "New URL."),
			"title", jsonobj.Obj("type", "string", "description", "New human-readable title. Omit to keep unchanged."),
			"related_time", jsonobj.Obj("type", "string", "description", "New related time. "+ISODateTimeDesc+" or "+DateOnlyDesc+". Omit to keep unchanged."),
			"locale_name", jsonobj.Obj("type", "string", "description", localeNameDesc),
		), []string{"id"}),
	),
	tool(
		...
		schema(jsonobj.Obj(
			"id", jsonobj.Obj("type", "string", "description", "ID of the nlog to update. Obtain from gkill_add_nlog response or gkill_get_kyous."),
			"title", jsonobj.Obj("type", "string", "description", "New expense/income description."),
			"amount", jsonobj.Obj("type", "integer", "description", "New monetary amount (integer only, e.g. -1500 for expense, 200 for income). Must be a valid integer."),
			"shop", jsonobj.Obj("type", "string", "description", "New shop/store name. Omit to keep unchanged."),
			"related_time", jsonobj.Obj("type", "string", "description", "New related time. "+ISODateTimeDesc+" or "+DateOnlyDesc+". Omit to keep unchanged."),
			"locale_name", jsonobj.Obj("type", "string", "description", localeNameDesc),
		), []string{"id"}),
	),
	tool(
		...
		schema(jsonobj.Obj(
			"id", jsonobj.Obj("type", "string", "description", "ID of the lantana to update. Obtain from gkill_add_lantana response or gkill_get_kyous."),
			"mood", jsonobj.Obj("type", "integer", "description", "New mood level: 0 (lowest) to 10 (highest). Must be an integer.", "minimum", 0, "maximum", 10),
			"related_time", jsonobj.Obj("type", "string", "description", "New related time. "+ISODateTimeDesc+" or "+DateOnlyDesc+". Omit to keep unchanged."),
			"locale_name", jsonobj.Obj("type", "string", "description", localeNameDesc),
		), []string{"id"}),
	),
	tool(
		...
		schema(jsonobj.Obj(
			"id", jsonobj.Obj("type", "string", "description", "ID of the timeis to update. Obtain from gkill_add_timeis response or gkill_get_kyous."),
			"title", jsonobj.Obj("type", "string", "description", "New activity title/label."),
			"start_time", jsonobj.Obj("type", "string", "description", "New start time. "+ISODateTimeDesc+" or "+DateOnlyDesc+". Omit to keep unchanged."),
			"end_time", jsonobj.Obj("type", jsonobj.Strings("string", "null"), "description", "New end time. "+ISODateTimeDesc+" or "+DateOnlyDesc+". Set this to close an open-ended (ongoing) timeis. Omit to keep unchanged. Pass null to CLEAR it and put the interval back to in-progress — omitting the field leaves the existing end time alone, so null is the only way to reopen a timeis that was already ended."),
			"locale_name", jsonobj.Obj("type", "string", "description", localeNameDesc),
		), []string{"id"}),
	),
	tool(
		...
		schema(jsonobj.Obj(
			"id", jsonobj.Obj("type", "string", "description", "ID of the mi to update. Obtain from gkill_add_mi response or gkill_get_kyous."),
			"title", jsonobj.Obj("type", "string", "description", "New task title."),
			"board_name", jsonobj.Obj("type", "string", "description", "New board name to move the task to. Any string accepted — non-existent names create new boards (unless allow_create_board is false). Omit to keep the current board unchanged."),
			"allow_create_board", jsonobj.Obj(
				"type", "boolean",
				"description",
				"Default: true — an unknown board_name silently creates a new board. "+
			...
			"is_checked", jsonobj.Obj("type", "boolean", "description", "Set to true to mark as completed, false to reopen. Omit to keep unchanged."),
			"limit_time", jsonobj.Obj("type", jsonobj.Strings("string", "null"), "description", "New deadline. "+ISODateTimeDesc+" or "+DateOnlyDesc+". Omit to keep unchanged; pass null to clear the deadline."),
			"estimate_start_time", jsonobj.Obj("type", jsonobj.Strings("string", "null"), "description", "New estimated start time. "+ISODateTimeDesc+" or "+DateOnlyDesc+". Omit to keep unchanged; pass null to clear it."),
			"estimate_end_time", jsonobj.Obj("type", jsonobj.Strings("string", "null"), "description", "New estimated end time. "+ISODateTimeDesc+" or "+DateOnlyDesc+". Omit to keep unchanged; pass null to clear it."),
			"locale_name", jsonobj.Obj("type", "string", "description", localeNameDesc),
		), []string{"id"}),
	),
	tool(
		...
		schema(jsonobj.Obj(
			"id", jsonobj.Obj("type", "string", "description", "ID of the kc to update. Obtain from gkill_add_kc response or gkill_get_kyous."),
			"title", jsonobj.Obj("type", "string", "description", "New measurement title (e.g., \"steps\", \"weight\")."),
			"num_value", jsonobj.Obj("type", "number", "description", "New numeric value. Integer or decimal (e.g., 10000, 72.5)."),
			"related_time", jsonobj.Obj("type", "string", "description", "New related time. "+ISODateTimeDesc+" or "+DateOnlyDesc+". Omit to keep unchanged."),
			"locale_name", jsonobj.Obj("type", "string", "description", localeNameDesc),
		), []string{"id"}),
	),
	tool(
		...
		schema(jsonobj.Obj(
			"id", jsonobj.Obj("type", "string", "description", "ID of the tag entity to update. This is the tag's own ID (added_tag.id), not the target entry's ID."),
			"tag", jsonobj.Obj("type", "string", "description", "New tag name string."),
			"locale_name", jsonobj.Obj("type", "string", "description", localeNameDesc),
		), []string{"id"}),
	),
	tool(
		...
		schema(jsonobj.Obj(
			"id", jsonobj.Obj("type", "string", "description", "ID of the text annotation entity to update. This is the text's own ID (added_text.id), not the target entry's ID."),
			"text", jsonobj.Obj("type", "string", "description", "New text annotation content. Supports multi-line."),
			"locale_name", jsonobj.Obj("type", "string", "description", localeNameDesc),
		), []string{"id"}),
	),
	tool(
		...
		"Undo a soft-delete: clear is_deleted on an entry so it shows up in searches again. "+
			...
			"Addressing: exactly one form is REQUIRED — either id + data_type (single entry) or targets[] (batch). The schema marks none of them required because it cannot express this either/or; a call with neither (or both) is rejected at runtime.",
		schema(jsonobj.Obj(
			"id", jsonobj.Obj("type", "string", "description", "ID of the soft-deleted entry to restore."),
			"targets", targetsProperty("restore"),
			"data_type", jsonobj.Obj(
				"type", "string",
				"description",
				"Data type of the entry. Must match the actual type of the entry. "+projectionVocabularyDesc,
				"enum", stringsToAny(EntityAndProjectionDataTypeValues),
			),
			"locale_name", jsonobj.Obj("type", "string", "description", localeNameDesc),
		), nil),
