...
INSERT INTO ` + sqlite3impl.QuoteIdent(dbName) + ` (
  ...
  ?
	...
	slog.Log(ctx, gkill_log.TraceSQL, "sql", "sql", fmt.Sprintf("%q", addKCInfoSQL))
...
INSERT INTO ` + sqlite3impl.QuoteIdent(k.dbName) + ` (
  ...
  ?
	...
	commitCacheRebuildIfSupported(k.kcRep)
	return nil
}
...
func (k *kcRepositoryCachedSQLite3Impl) AddKCInfo(ctx context.Context, kc KC) error {
	...
	slog.Log(ctx, gkill_log.TraceSQL, "sql", "sql", fmt.Sprintf("%q", k.addKCInfoSQL), "params", fmt.Sprintf("%q", fmt.Sprint(queryArgs)))
