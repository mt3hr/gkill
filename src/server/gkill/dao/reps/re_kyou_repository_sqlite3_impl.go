...
CREATE TABLE IF NOT EXISTS "REKYOU" (
  ...
  UPDATE_USER NOT NULL 
	...
	if err := sqlite3impl.EnsureUnixepochIndex(ctx, db, "REKYOU", "RELATED_TIME"); err != nil {
		return nil, err
	}
...
`
	...
	// 最新版に絞ってもrepをまたいだ同一版が複数返りうるので、UpdateTimeが最大のものを選ぶ。
	// 格納順の先頭を返すと、どれが返るかがSQLiteの都合で決まってしまう。
	latestReKyou := slices.MaxFunc(reKyous, func(a ReKyou, b ReKyou) int {
		return a.UpdateTime.Compare(b.UpdateTime)
	})
	return &latestReKyou, nil
}
...
func (r *reKyouRepositorySQLite3Impl) AddReKyouInfo(ctx context.Context, rekyou ReKyou) error {
	...
	return insertReKyouRow(ctx, db, rekyou)
}

// insertReKyouRow は ReKyou を1行 INSERT する。契約は AddReKyouInfo と同じで、書き込み先だけを引数で受ける。
//
// rep 自身の接続（AddReKyouInfo）にも、commit_tx が書き込み rep のファイルを ATTACH した1接続の
// トランザクション（commit_tx.go）にも同じ SQL を打てるようにするための切り出し。
// INSERT の列と検査はここだけに置き、AddXxxInfo 側へ複製しないこと（ずれると tx 経由の追記だけ壊れる）。
func insertReKyouRow(ctx context.Context, db sqlite3impl.Preparer, rekyou ReKyou) error {
...
func (r *reKyouRepositorySQLite3Impl) GetLatestDataRepositoryAddress(ctx context.Context, updateCache bool) ([]gkill_cache.LatestDataRepositoryAddress, error) {
	var err error
	var db *sql.DB
	if r.fullConnect {
		db = r.db
