...
errloop:
	for {
		select {
		case e := <-errch:
			err = errors.Join(err, fmt.Errorf("error at find kmemo: %w", e))
			existErr = true
		default:
			break errloop
		}
...
errloop:
	for {
		select {
		case e := <-errch:
			err = errors.Join(err, fmt.Errorf("error at get kmemo: %w", e))
			existErr = true
		default:
			break errloop
		}
...
loop:
	for {
		...
		case matchKmemoInRep := <-ch:
			...
			if matchKmemo != nil {
				if matchKmemoInRep.UpdateTime.After(matchKmemo.UpdateTime) {
...
errloop:
	for {
		select {
		case e := <-errch:
			err = errors.Join(err, fmt.Errorf("error at find get kmemo histories: %w", e))
			existErr = true
		default:
			break errloop
		}
...
loop:
	...
	slices.SortFunc(kyouHistoriesList, func(a, b Kmemo) int {
		return b.UpdateTime.Compare(a.UpdateTime)
	})
...
func (k KmemoRepositories) GetLatestDataRepositoryAddress(ctx context.Context, updateCache bool) ([]gkill_cache.LatestDataRepositoryAddress, error) {
	allAddrs := []gkill_cache.LatestDataRepositoryAddress{}
	existErr := false
	var err error
	wg := &sync.WaitGroup{}
	ch := make(chan []gkill_cache.LatestDataRepositoryAddress, len(k))
	errch := make(chan error, len(k))
	defer close(ch)
	defer close(errch)
