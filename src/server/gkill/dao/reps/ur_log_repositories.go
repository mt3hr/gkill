...
errloop:
	for {
		select {
		case e := <-errch:
			err = errors.Join(err, fmt.Errorf("error at find urlog: %w", e))
			existErr = true
		default:
			break errloop
		}
...
errloop:
	for {
		select {
		case e := <-errch:
			err = errors.Join(err, fmt.Errorf("error at get urlog: %w", e))
			existErr = true
		default:
			break errloop
		}
...
loop:
	for {
		...
		case matchURLogInRep := <-ch:
			...
			if matchURLog != nil {
				if matchURLogInRep.UpdateTime.After(matchURLog.UpdateTime) {
...
errloop:
	for {
		select {
		case e := <-errch:
			err = errors.Join(err, fmt.Errorf("error at find get urlog histories: %w", e))
			existErr = true
		default:
			break errloop
		}
...
func (u URLogRepositories) GetLatestDataRepositoryAddress(ctx context.Context, updateCache bool) ([]gkill_cache.LatestDataRepositoryAddress, error) {
	allAddrs := []gkill_cache.LatestDataRepositoryAddress{}
	existErr := false
	var err error
	wg := &sync.WaitGroup{}
	ch := make(chan []gkill_cache.LatestDataRepositoryAddress, len(u))
	errch := make(chan error, len(u))
	defer close(ch)
	defer close(errch)
