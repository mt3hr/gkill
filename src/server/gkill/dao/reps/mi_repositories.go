...
errloop:
	for {
		select {
		case e := <-errch:
			err = errors.Join(err, fmt.Errorf("error at find mi: %w", e))
			existErr = true
		default:
			break errloop
		}
...
errloop:
	for {
		select {
		case e := <-errch:
			err = errors.Join(err, fmt.Errorf("error at get mi: %w", e))
			existErr = true
		default:
			break errloop
		}
...
loop:
	for {
		...
		case matchMiInRep := <-ch:
			...
			if matchMi != nil {
				if matchMiInRep.UpdateTime.After(matchMi.UpdateTime) {
...
errloop:
	for {
		select {
		case e := <-errch:
			err = errors.Join(err, fmt.Errorf("error at find get mi histories: %w", e))
			existErr = true
		default:
			break errloop
		}
...
func (m MiRepositories) GetLatestDataRepositoryAddress(ctx context.Context, updateCache bool) ([]gkill_cache.LatestDataRepositoryAddress, error) {
	allAddrs := []gkill_cache.LatestDataRepositoryAddress{}
	existErr := false
	var err error
	wg := &sync.WaitGroup{}
	ch := make(chan []gkill_cache.LatestDataRepositoryAddress, len(m))
	errch := make(chan error, len(m))
	defer close(ch)
	defer close(errch)
