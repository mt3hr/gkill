...
// KCTempRepository は、まだ確定していないKC（数値記録）を置いておく一時リポジトリが満たす契約です。
//
// 行は (txID, userID, device) でスコープされます。トランザクション中の書き込みは
// AddKCInfo でここに溜まり、/api/commit_tx が GetKCsByTXID で取り出して
// 本体のリポジトリへ書き写し、/api/discard_tx が DeleteByTXID で捨てます。
// 実体は利用者ごとの一時DB（既定はインメモリ）上のKC表ひとつです。
type KCTempRepository interface {
	// FindKyous の契約は Repository.FindKyous を参照。
	//
	// ただしsqlite3実装は基底のkc実装へ構造体変換で委譲しており、変換後は
	// 一時DBのハンドルではなく "temp_db" というファイル名でDBを開き直すため、
	// 一時表は読めずエラーになります。未確定データの取り出しには GetKCsByTXID を使ってください。
	...
	// UnWrapTyped は UnWrap の型付き版で、KCTempRepository のまま平坦化します。
