...
func (n NlogRepositories) CommitCacheRebuild() {
	for _, rep := range n {
...
errloop:
	for {
		select {
		case e := <-errch:
			err = errors.Join(err, fmt.Errorf("error at find nlog: %w", e))
			existErr = true
		default:
			break errloop
		}
...
errloop:
	for {
		select {
		case e := <-errch:
			err = errors.Join(err, fmt.Errorf("error at get nlog: %w", e))
			existErr = true
		default:
			break errloop
		}
...
errloop:
	for {
		select {
		case e := <-errch:
			err = errors.Join(err, fmt.Errorf("error at find get nlog histories: %w", e))
			existErr = true
		default:
			break errloop
		}
...
loop:
	...
	slices.SortFunc(kyouHistoriesList, func(a, b Nlog) int {
		return b.UpdateTime.Compare(a.UpdateTime)
	})
...
func (n NlogRepositories) GetLatestDataRepositoryAddress(ctx context.Context, updateCache bool) ([]gkill_cache.LatestDataRepositoryAddress, error) {
	allAddrs := []gkill_cache.LatestDataRepositoryAddress{}
	existErr := false
	var err error
	wg := &sync.WaitGroup{}
	ch := make(chan []gkill_cache.LatestDataRepositoryAddress, len(n))
	errch := make(chan error, len(n))
	defer close(ch)
	defer close(errch)
