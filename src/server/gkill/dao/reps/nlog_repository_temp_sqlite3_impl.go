...
CREATE TABLE IF NOT EXISTS "NLOG" (
  ...
  TX_ID NOT NULL
	...
	// ID先頭の既存索引では効かない。
	if err := sqlite3impl.EnsureTxIDIndex(ctx, db, "NLOG"); err != nil {
		return nil, err
	}
...
func (n *nlogTempRepositorySQLite3Impl) GetLatestDataRepositoryAddress(ctx context.Context, updateCache bool) ([]gkill_cache.LatestDataRepositoryAddress, error) {
	return nil, fmt.Errorf("not implements nlogTempRepositorySQLite3Impl.GetLatestDataRepositoryAddress")
}
