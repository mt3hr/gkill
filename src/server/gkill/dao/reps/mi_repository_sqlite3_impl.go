...
func (m *miRepositorySQLite3Impl) AddMiInfo(ctx context.Context, mi Mi) error {
	...
	return insertMiRow(ctx, db, mi)
}

// insertMiRow は Mi を1行 INSERT する。契約は AddMiInfo と同じで、書き込み先だけを引数で受ける。
//
// rep 自身の接続（AddMiInfo）にも、commit_tx が書き込み rep のファイルを ATTACH した1接続の
// トランザクション（commit_tx.go）にも同じ SQL を打てるようにするための切り出し。
// INSERT の列と検査はここだけに置き、AddXxxInfo 側へ複製しないこと（ずれると tx 経由の追記だけ壊れる）。
func insertMiRow(ctx context.Context, db sqlite3impl.Preparer, mi Mi) error {
