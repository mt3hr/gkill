...
INSERT INTO ` + sqlite3impl.QuoteIdent(dbName) + ` (
  ...
  ?
	...
	slog.Log(ctx, gkill_log.TraceSQL, "sql", "sql", fmt.Sprintf("%q", addKmemoInfoSQL))
...
INSERT INTO ` + sqlite3impl.QuoteIdent(k.dbName) + ` (
  ...
  ?
	...
	commitCacheRebuildIfSupported(k.kmemoRep)
	return nil
}
...
func (k *kmemoRepositoryCachedSQLite3Impl) AddKmemoInfo(ctx context.Context, kmemo Kmemo) error {
	...
	slog.Log(ctx, gkill_log.TraceSQL, "sql", "sql", fmt.Sprintf("%q", k.addKmemoInfoSQL), "params", fmt.Sprintf("%q", fmt.Sprint(queryArgs)))
