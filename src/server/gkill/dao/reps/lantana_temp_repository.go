...
// LantanaTempRepository は、まだ確定していないLantana（気分値）を置いておく一時リポジトリが満たす契約です。
//
// 行は (txID, userID, device) でスコープされます。トランザクション中の書き込みは
// AddLantanaInfo でここに溜まり、/api/commit_tx が GetLantanasByTXID で取り出して
// 本体のリポジトリへ書き写し、/api/discard_tx が DeleteByTXID で捨てます。
// 実体は利用者ごとの一時DB（既定はインメモリ）上のLANTANA表ひとつです。
type LantanaTempRepository interface {
	// FindKyous の契約は Repository.FindKyous を参照。
	//
	// ただしsqlite3実装は基底のlantana実装へ構造体変換で委譲しており、変換後は
	// 一時DBのハンドルではなく "lantana_temp" というファイル名でDBを開き直すため、
	// 一時表は読めずエラーになります。未確定データの取り出しには GetLantanasByTXID を使ってください。
	...
	// sqlite3実装は固定値 "lantana_temp" を返します。
	...
	// GetKyousByTXID は未確定LantanaをKyouとして返す想定のメソッドです。
	//
	// ただしsqlite3実装のSQLが一時表に無い TARGET_REP_NAME 列を参照しているため、
	// 現状は必ずエラーになります。コミット処理は GetLantanasByTXID を使っており、
	// 本メソッドの呼び出し元はありません。
	GetKyousByTXID(ctx context.Context, txID string, userID string, device string) ([]Kyou, error)

	// GetLantanasByTXID は txID・userID・device の3つすべてに一致する未確定Lantanaを返します。
	//
	// 一致する行が無ければ空スライスを返します（エラーではありません）。
	// RepName は一時rep名、DataType は "lantana" で埋めます。
	...
	// UnWrapTyped は UnWrap の型付き版で、LantanaTempRepository のまま平坦化します。
