...
func newGranularReKyouFixture(t *testing.T) (*GkillRepositories, string, string, string) {
	...
	pixelRep, err := NewReKyouRepositorySQLite3Impl(ctx, filepath.Join(dir, "ReKyou_TestPhoneA_20250919.db"), true, repositories)
	if err != nil {
	...
	phoneRep, err := NewReKyouRepositorySQLite3Impl(ctx, filepath.Join(dir, "ReKyou_TestPhone_20260226.db"), true, repositories)
	if err != nil {
		t.Fatalf("failed to create phone rekyou repo: %v", err)
	}
	...
	rekyouLatest.RepName = phoneRepName
	...
	originalReKyouReps := ReKyouRepositories{
		ReKyouRepositories: []ReKyouRepository{pixelRep, phoneRep},
	...
	return repositories, pixelRepName, phoneRepName, sharedID
}
...
func TestGkillRepositories_UpdateCacheBuildsReKyouOnFirstPass(t *testing.T) {
	...
	repositories, pixelRepName, phoneRepName, sharedID := newGranularReKyouFixture(t)
	...
	if !slices.Contains(allRepNames, phoneRepName) {
		t.Fatalf("GetAllRepNames() missing %s: %#v", phoneRepName, allRepNames)
	}
	...
	if addr.LatestDataRepositoryName != phoneRepName {
		t.Fatalf("LatestDataRepositoryName = %s, want %s", addr.LatestDataRepositoryName, phoneRepName)
	}
	...
	if got.RepName != phoneRepName {
		t.Fatalf("GetKyou().RepName = %s, want %s", got.RepName, phoneRepName)
	}
}
