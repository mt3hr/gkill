...
INSERT INTO ` + sqlite3impl.QuoteIdent(dbName) + ` (
  ...
  ?
	...
	gkill_log.LogSQL(ctx, addLantanaInfoSQL)
...
INSERT INTO ` + sqlite3impl.QuoteIdent(l.dbName) + ` (
  ...
  ?
	...
	commitCacheRebuildIfSupported(l.lantanaRep)
	return nil
}
...
`
	...
	// 最新版に絞ってもrepをまたいだ同一版が複数返りうるので、UpdateTimeが最大のものを選ぶ。
	// 格納順の先頭を返すと、どれが返るかがSQLiteの都合で決まってしまう。
	latestLantana := slices.MaxFunc(lantanas, func(a Lantana, b Lantana) int {
		return a.UpdateTime.Compare(b.UpdateTime)
	})
	return &latestLantana, nil
}
...
func (l *lantanaRepositoryCachedSQLite3Impl) AddLantanaInfo(ctx context.Context, lantana Lantana) error {
	...
	gkill_log.LogSQLParams(ctx, l.addLantanaInfoSQL, queryArgs)
...
func (l *lantanaRepositoryCachedSQLite3Impl) GetLatestDataRepositoryAddress(ctx context.Context, updateCache bool) ([]gkill_cache.LatestDataRepositoryAddress, error) {
	...
	// （包んでいるのは集約なので "LantanaReps" という実在しない名前が返る）。ADR-0210。
	sql := `
...
FROM ` + sqlite3impl.QuoteIdent(l.dbName) + ` AS T
WHERE T.UPDATE_TIME_UNIX = (SELECT MAX(UPDATE_TIME_UNIX) FROM ` + sqlite3impl.QuoteIdent(l.dbName) + ` AS INNER_TABLE WHERE INNER_TABLE.ID = T.ID)
`
	stmt, err := l.cachedDB.PrepareContext(ctx, sql)
	if err != nil {
		return nil, err
