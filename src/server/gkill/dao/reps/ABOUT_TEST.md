...
| `idf_thumb_vips_test.go` | 静止画のサムネイルを vips（libvips の CLI）で作る経路。EXIF の向きで起こし Orientation タグを落とすこと、入力の隣に書かないこと、vips が失敗したら Go 経路で作って印を残さないこと、3経路とも失敗したときの印に vips のエラーが包まれること、Go で読めて小さい画像は vips を飛ばす振り分け（形式別の画素数上限）。vips が無い環境は vips を要する検査を Skip（[ADR-0222](../../../../../documents/adr/0222-thumbnail-prefers-vips-cli-then-native-then-ffmpeg.md)） |
| `idf_thumb_orientation_test.go` | EXIF Orientation 1/3/6/8 × 正方形・横長・縦長の要求サイズで、Go 経路のサムネイルが正しい向き・寸法になること（手組みの APP1 セグメント）。回転で軸が入れ替わる向きの寸法も固定 |
| `idf_thumb_external_args_test.go` | ffmpeg / vips に渡す引数列。`-nostdin`、静止画の ffmpeg は復号・フィルタ・符号化とも1スレッドで動画は復号スレッドを絞らないこと、vips の出力が絶対パス・`.jpg` 終端・`[Q=<品質>,strip]` であること |
| `idf_thumb_scale_bench_test.go` | ベンチマーク（テストは無い）。縮小カーネル（CatmullRom / BiLinear / ApproxBiLinear）・EXIF 回転の位置・vips と Go の端から端までの比率。`go test -run '^| `idf_kyou_repository_batch_test.go` | IDFKyou のバッチ処理 |
...
npm run test_server
```
 -bench Thumb -benchmem ./gkill/dao/reps/` |
