...
func TestTagRepositoriesGetTagPicksLatestAcrossReps(t *testing.T) {
	...
	oldVersion := makeTag("cross-rep-tag", "cross-rep-target", "クロスレップ旧版タグ")
	newVersion := makeTag("cross-rep-tag", "cross-rep-target", "クロスレップ新版タグ")
	...
	for _, order := range []string{"old_first", "new_first"} {
		t.Run(order, func(t *testing.T) {
			...
			if got.Tag != "クロスレップ新版タグ" {
				t.Errorf("Tag = %q, want %q (latest version)", got.Tag, "クロスレップ新版タグ")
			}
		})
	}
}
