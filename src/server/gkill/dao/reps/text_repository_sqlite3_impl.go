...
CREATE TABLE IF NOT EXISTS "TEXT" (
  ...
  UPDATE_USER NOT NULL 
	...
	gkill_log.LogSQL(ctx, indexTargetIDSQL)
	...
	gkill_log.LogSQL(ctx, indexIDUpdateTimeSQL)
...
func (t *textRepositorySQLite3Impl) AddTextInfo(ctx context.Context, text Text) error {
	...
	return insertTextRow(ctx, db, text)
}

// insertTextRow は Text を1行 INSERT する。契約は AddTextInfo と同じで、書き込み先だけを引数で受ける。
//
// rep 自身の接続（AddTextInfo）にも、commit_tx が書き込み rep のファイルを ATTACH した1接続の
// トランザクション（commit_tx.go）にも同じ SQL を打てるようにするための切り出し。
// INSERT の列と検査はここだけに置き、AddXxxInfo 側へ複製しないこと（ずれると tx 経由の追記だけ壊れる）。
func insertTextRow(ctx context.Context, db sqlite3impl.Preparer, text Text) error {
...
func (t *textRepositorySQLite3Impl) GetLatestDataRepositoryAddress(ctx context.Context, updateCache bool) ([]gkill_cache.LatestDataRepositoryAddress, error) {
	var err error
	var db *sql.DB
	if t.fullConnect {
		db = t.db
