...
`
	...
	gkill_log.LogSQL(ctx, latestIndexSQL)
	...
	if err := sqlite3impl.EnsureUnixepochIndex(ctx, db, "IDF", "RELATED_TIME"); err != nil {
		return nil, err
	}
...
// （2巡目の指摘）。検索のたびにディレクトリを全走査して未採番を数えるのは
...
// 保持しており、その内側でさらに取ると入れ子になってプールが枯渇します（ADR-0206）。
...
func (i *idfKyouRepositorySQLite3Impl) GetLatestDataRepositoryAddress(ctx context.Context, updateCache bool) ([]gkill_cache.LatestDataRepositoryAddress, error) {
	var err error
	var db *sql.DB
	if i.fullConnect {
		db = i.db
