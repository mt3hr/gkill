...
errloop:
	for {
		select {
		case e := <-errch:
			err = errors.Join(err, fmt.Errorf("error at find lantana: %w", e))
			existErr = true
		default:
			break errloop
		}
...
errloop:
	for {
		select {
		case e := <-errch:
			err = errors.Join(err, fmt.Errorf("error at get lantana: %w", e))
			existErr = true
		default:
			break errloop
		}
...
loop:
	for {
		...
		case matchLantanaInRep := <-ch:
			...
			if matchLantana != nil {
				if matchLantanaInRep.UpdateTime.After(matchLantana.UpdateTime) {
...
errloop:
	for {
		select {
		case e := <-errch:
			err = errors.Join(err, fmt.Errorf("error at find get lantana histories: %w", e))
			existErr = true
		default:
			break errloop
		}
...
loop:
	...
	slices.SortFunc(kyouHistoriesList, func(a, b Lantana) int {
		return b.UpdateTime.Compare(a.UpdateTime)
	})
