...
func (k KCRepositories) CommitCacheRebuild() {
	for _, rep := range k {
...
errloop:
	for {
		select {
		case e := <-errch:
			err = errors.Join(err, fmt.Errorf("error at find kc: %w", e))
			existErr = true
		default:
			break errloop
		}
...
errloop:
	for {
		select {
		case e := <-errch:
			err = errors.Join(err, fmt.Errorf("error at get kc: %w", e))
			existErr = true
		default:
			break errloop
		}
...
errloop:
	for {
		select {
		case e := <-errch:
			err = errors.Join(err, fmt.Errorf("error at find get kc histories: %w", e))
			existErr = true
		default:
			break errloop
		}
...
loop:
	...
	slices.SortFunc(kyouHistoriesList, func(a, b KC) int {
		return b.UpdateTime.Compare(a.UpdateTime)
	})
...
func (k KCRepositories) GetLatestDataRepositoryAddress(ctx context.Context, updateCache bool) ([]gkill_cache.LatestDataRepositoryAddress, error) {
