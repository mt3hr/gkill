...
const CURRENT_SCHEMA_VERSION_MIREKYOU_REPOSITORY_SQLITE3IMPL_DAO = "1.0.0"
...
func NewMiReKyouRepositorySQLite3Impl(ctx context.Context, filename string, fullConnect bool, reps *GkillRepositories) (MiReKyouRepository, error) {
	...
	if isOld, oldVerDAO, err := checkAndResolveDataSchemaMiReKyouRepositorySQLite3Impl(ctx, db); err != nil {
		return nil, err
	} else if isOld {
		if oldVerDAO != nil {
...
func (m *miReKyouRepositorySQLite3Impl) GetMiReKyou(ctx context.Context, id string, updateTime *time.Time) (*MiReKyou, error) {
	...
	// どれが最新かは保証されない（指摘 H-07。他の型別 GetXxx は全て
...
func checkAndResolveDataSchemaMiReKyouRepositorySQLite3Impl(ctx context.Context, db *sqllib.DB) (isOld bool, oldVerDAO MiReKyouRepository, err error) {
	schemaVersionKey := "SCHEMA_VERSION_MIREKYOU"
	currentSchemaVersion := CURRENT_SCHEMA_VERSION_MIREKYOU_REPOSITORY_SQLITE3IMPL_DAO
