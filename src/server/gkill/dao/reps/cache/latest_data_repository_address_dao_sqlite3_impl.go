...
func NewLatestDataRepositoryAddressSQLite3Impl(userID string, db *sql.DB, mutex *sync.RWMutex) (LatestDataRepositoryAddressDAO, error) {
	...
	sql := `
CREATE TABLE IF NOT EXISTS ` + sqlite3impl.QuoteIdent(latestDataRepositoryAddress.tableName) + ` (
  IS_DELETED NOT NULL,
  TARGET_ID_IN_DATA,
  TARGET_ID NOT NULL,
  ...
  PRIMARY KEY(TARGET_ID)
	...
	gkill_log.LogIndexSQL(ctx, repNameIndexSQL)
...
WHERE TARGET_ID = ?`
	...
	insertSQL := `
INSERT INTO ` + sqlite3impl.QuoteIdent(l.tableName) + ` (
  IS_DELETED,
  TARGET_ID,
  TARGET_ID_IN_DATA,
  LATEST_DATA_REPOSITORY_NAME,
...
`
	...
	gkill_log.LogSQL(ctx, deleteSQL)
	...
	gkill_log.LogSQLQuery(ctx, deleteSQL, deleteQueryArgs)
	...
	gkill_log.LogSQLQuery(ctx, insertSQL, insertQueryArgs)
