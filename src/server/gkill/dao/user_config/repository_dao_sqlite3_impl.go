...
// checkUseToWriteRepositoryCount は、種別ごとに書き込み用リポジトリが1つだけかを検査します。
//
// 真偽値の比較を CAST(... AS INTEGER) = 1 で書いているのは、REPOSITORY の各列に型名が無く
// affinity が無いためです。書き込み経路によっては INTEGER ではなく TEXT の "0"/"1" が
// 入っている行があり、素朴に `= TRUE`（＝ `= 1`）と書くと SQLite は TEXT と INTEGER を
// 別の型として比較して必ず偽になり、その行を数え落とします。
