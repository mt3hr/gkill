...
func (uc *UsecaseContext) UpdateMiReKyou(ctx context.Context, repositories *reps.GkillRepositories, userID, device, localeName string, mirekyou reps.MiReKyou, txID *string) ([]*message.GkillError, error) {
	...
	if txID == nil {
		...
		// 実体は WriteMiReKyouRep（書き込み先rep）へ足したので、キャッシュ表の REP_NAME もそこに合わせる。
		...
		if writeRepName, repNameErr := repositories.WriteMiReKyouRep.GetRepName(ctx); repNameErr == nil {
			mirekyou.RepName = writeRepName
		} else {
			mirekyou.RepName = ""
		}
		err = repositories.WriteThroughMiReKyouCache(ctx, mirekyou)
