...
func (uc *UsecaseContext) AddTimeIs(ctx context.Context, repositories *reps.GkillRepositories, userID, device, localeName string, timeis reps.TimeIs, txID *string) ([]*message.GkillError, error) {
	...
	if err != nil {
		...
		slog.Log(ctx, gkill_log.Debug, "error at get timeis user id", "error", fmt.Sprintf("%q", err))
	...
	if existTimeIs != nil {
		...
		slog.Log(ctx, gkill_log.Debug, "exist timeis id", "error", fmt.Sprintf("%q", err))
		gkillErrors = append(gkillErrors, &message.GkillError{
	...
	if txID == nil && repositories.WriteTimeIsRep == nil {
		gkillErrors = append(gkillErrors, writeRepMissingError(localeName, "FAILED_ADD_TIMEIS_MESSAGE", "timeis"))
		return gkillErrors, nil
	}
	if txID == nil {
		...
		if err != nil {
			...
			slog.Log(ctx, gkill_log.Debug, "error at add timeis user id", "error", fmt.Sprintf("%q", err))
			gkillErrors = append(gkillErrors, &message.GkillError{
...
func (uc *UsecaseContext) UpdateTimeIs(ctx context.Context, repositories *reps.GkillRepositories, userID, device, localeName string, timeis reps.TimeIs, txID *string) ([]*message.GkillError, error) {
	...
	if existTimeIs == nil {
		...
		slog.Log(ctx, gkill_log.Debug, "not exist timeis id", "error", fmt.Sprintf("%q", err))
		gkillErrors = append(gkillErrors, &message.GkillError{
	...
	// 書き込み先 rep が未設定なら nil ポインタ参照で落ちる前に、設定不備として返す（reason: write_rep_missing）。
	if txID == nil && repositories.WriteTimeIsRep == nil {
		gkillErrors = append(gkillErrors, writeRepMissingError(localeName, "FAILED_UPDATE_TIMEIS_MESSAGE", "timeis"))
		return gkillErrors, nil
	}
	if txID == nil {
		err = repositories.WriteTimeIsRep.AddTimeIsInfo(ctx, timeis)
		if err != nil {
			...
			slog.Log(ctx, gkill_log.Debug, "error at update timeis user id", "error", fmt.Sprintf("%q", err))
			gkillErrors = append(gkillErrors, &message.GkillError{
