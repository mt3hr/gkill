...
func (uc *UsecaseContext) AddURLog(ctx context.Context, repositories *reps.GkillRepositories, userID, device, localeName string, urlog reps.URLog, txID *string) ([]*message.GkillError, error) {
	...
	if err != nil {
		...
		slog.Log(ctx, gkill_log.Debug, "error at get urlog user id", "error", fmt.Sprintf("%q", err))
	...
	if existURLog != nil {
		...
		slog.Log(ctx, gkill_log.Debug, "exist urlog id", "error", fmt.Sprintf("%q", err))
		gkillErrors = append(gkillErrors, &message.GkillError{
	...
	if txID == nil && repositories.WriteURLogRep == nil {
		gkillErrors = append(gkillErrors, writeRepMissingError(localeName, "FAILED_ADD_URLOG_MESSAGE", "urlog"))
		return gkillErrors, nil
	}
	if txID == nil {
		...
		if err != nil {
			...
			slog.Log(ctx, gkill_log.Debug, "error at add urlog user id", "error", fmt.Sprintf("%q", err))
			gkillErrors = append(gkillErrors, &message.GkillError{
...
func (uc *UsecaseContext) UpdateURLog(ctx context.Context, repositories *reps.GkillRepositories, userID, device, localeName string, urlog reps.URLog, txID *string) ([]*message.GkillError, error) {
	...
	if existURLog == nil {
		...
		slog.Log(ctx, gkill_log.Debug, "not exist urlog id", "error", fmt.Sprintf("%q", err))
		gkillErrors = append(gkillErrors, &message.GkillError{
	...
	// 書き込み先 rep が未設定なら nil ポインタ参照で落ちる前に、設定不備として返す（reason: write_rep_missing）。
	if txID == nil && repositories.WriteURLogRep == nil {
		gkillErrors = append(gkillErrors, writeRepMissingError(localeName, "FAILED_UPDATE_URLOG_MESSAGE", "urlog"))
		return gkillErrors, nil
	}
	if txID == nil {
		err = repositories.WriteURLogRep.AddURLogInfo(ctx, urlog)
		if err != nil {
			...
			slog.Log(ctx, gkill_log.Debug, "error at update urlog user id", "error", fmt.Sprintf("%q", err))
			gkillErrors = append(gkillErrors, &message.GkillError{
