...
func (uc *UsecaseContext) UpdateMi(ctx context.Context, repositories *reps.GkillRepositories, userID, device, localeName string, mi reps.Mi, txID *string) ([]*message.GkillError, error) {
	...
	if txID == nil {
		...
		// 実体は WriteMiRep（書き込み先rep）へ足したので、キャッシュ表の REP_NAME もそこに合わせる。
		...
		if writeRepName, repNameErr := repositories.WriteMiRep.GetRepName(ctx); repNameErr == nil {
			mi.RepName = writeRepName
		} else {
			mi.RepName = ""
		}
		err = repositories.WriteThroughMiCache(ctx, mi)
