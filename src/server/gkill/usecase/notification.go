...
func (uc *UsecaseContext) AddNotification(ctx context.Context, repositories *reps.GkillRepositories, userID, device, localeName string, notification reps.Notification, txID *string) (*reps.Notification, []*message.GkillError, error) {
	...
	if err != nil {
		...
		slog.Log(ctx, gkill_log.Debug, "error at get notification user id", "error", fmt.Sprintf("%q", err))
	...
	if existNotification != nil {
		...
		slog.Log(ctx, gkill_log.Debug, "exist notification id", "error", fmt.Sprintf("%q", err))
		gkillErrors = append(gkillErrors, &message.GkillError{
	...
	if txID == nil {
		...
		if err != nil {
			...
			slog.Log(ctx, gkill_log.Debug, "error at add notification user id", "error", fmt.Sprintf("%q", err))
			gkillErrors = append(gkillErrors, &message.GkillError{
...
func (uc *UsecaseContext) UpdateNotification(ctx context.Context, repositories *reps.GkillRepositories, userID, device, localeName string, notification reps.Notification, txID *string) (*reps.Notification, []*message.GkillError, error) {
	...
	if existNotification == nil {
		...
		slog.Log(ctx, gkill_log.Debug, "not exist notification id", "error", fmt.Sprintf("%q", err))
		gkillErrors = append(gkillErrors, &message.GkillError{
	...
	if txID == nil {
		...
		if err != nil {
			...
			slog.Log(ctx, gkill_log.Debug, "error at update notification user id", "error", fmt.Sprintf("%q", err))
			gkillErrors = append(gkillErrors, &message.GkillError{
		...
		// 実体は WriteNotificationRep（書き込み先rep）へ足したので、キャッシュ表の REP_NAME もそこに合わせる。
		...
		if writeRepName, repNameErr := repositories.WriteNotificationRep.GetRepName(ctx); repNameErr == nil {
			notification.RepName = writeRepName
		} else {
...
func (uc *UsecaseContext) GetNotificationsByTargetID(ctx context.Context, repositories *reps.GkillRepositories, userID, device, localeName string, targetID string) ([]reps.Notification, []*message.GkillError, error) {
	...
	if err != nil {
		...
		slog.Log(ctx, gkill_log.Debug, "error at get notifications by target id user id", "error", fmt.Sprintf("%q", err))
		gkillErrors = append(gkillErrors, &message.GkillError{
...
func (uc *UsecaseContext) GetNotificationHistoriesByNotificationID(ctx context.Context, repositories *reps.GkillRepositories, userID, device, localeName string, id string, updateTime *time.Time) ([]reps.Notification, []*message.GkillError, error) {
	...
	if err != nil {
		...
		slog.Log(ctx, gkill_log.Debug, "error at get notification histories by notification id user id", "error", fmt.Sprintf("%q", err))
		gkillErrors = append(gkillErrors, &message.GkillError{
