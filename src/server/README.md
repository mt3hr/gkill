...
└── gkill/
    ...
    │   ├── gkill_server_api/  # HTTP ハンドラ（119 handle_*.go + 認証・ルーティング）
    │   ├── embed/             # ビルド生成物（Vue SPA 埋め込み）
    │   ├── find/              # 検索クエリ型
    │   ├── find_word/         # ワード検索の Go 側判定（プラグイン SDK と共用）
    ...
    │   └── safefetch/         # SSRF 対策の共有フェッチ（利用者入力 URL の取得防御）
...
**前提条件:**
- Go 1.26.6+（`src/server/go.mod` が `go 1.26.6` を宣言）
...
- [api/README.md](gkill/api/README.md) — HTTP API 共通基盤・ハンドラ層（全97エンドポイント一覧）
- [api/gkill_server_api/README.md](gkill/api/gkill_server_api/README.md) — HTTP ハンドラ（165ファイル）
