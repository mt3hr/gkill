        ...
        <v-card variant="flat" class="pa-2">
          <v-card-title>
            <v-row class="pa-0 ma-0">
              <v-col cols="auto" class="pa-0 ma-0">
                <span>{{ i18n.global.t('TEXT_CONTEXTMENU_HISTORIES') }}</span>
              </v-col>
              <v-spacer />
              <v-col cols="auto" class="pa-0 ma-0">
          ...
          <v-card v-if="show_kyou">
            <KyouView :application_config="application_config" :gkill_api="gkill_api"
              :is_image_request_to_thumb_size="false" :highlight_targets="text_highlight_targets" :is_image_view="false"
              :kyou="kyou" :show_checkbox="false" :show_content_only="false"
              :show_mi_create_time="true" :show_mi_estimate_end_time="true" :show_mi_estimate_start_time="true"
              :show_mi_limit_time="true" :show_timeis_elapsed_time="true" :show_timeis_playing_end_button="true"
              :height="'unset'" :width="'100%'" :enable_context_menu="enable_context_menu" :enable_dialog="enable_dialog"
...
import { useTextHistoriesDialog } from '@/classes/use-text-histories-dialog'

const props = defineProps<TextHistoriesDialogProps>()
const emits = defineEmits<KyouDialogEmits>()
const { crudRelayHandlers, text_highlight_targets, is_show_dialog, ui, show_kyou, show, hide } = useTextHistoriesDialog({ props, emits })
