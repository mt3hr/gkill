<template>
    <v-card elevation="0" @contextmenu.prevent="show_context_menu" :width="width" :height="height">
        ...
        <div v-if="kyou.typed_timeis && !kyou.typed_timeis.end_time && show_timeis_elapsed_time">{{
            i18n.global.t("PLAYING_TITLE")
            }}</div>
        <v-row v-if="show_timeis_playing_end_button && kyou.typed_timeis && !kyou.typed_timeis.end_time"
            class="pa-0 ma-0">
            <v-spacer cols="auto" />
        ...
        <EndTimeIsPlayingDialog :application_config="application_config" :gkill_api="gkill_api"
            :highlight_targets="highlight_targets" :kyou="kyou"
            ref="end_timeis_playing_dialog" :enable_context_menu="enable_context_menu" :enable_dialog="enable_dialog"
            v-on="crudRelayHandlers" />
    </v-card>
</template>
...
import EndTimeIsPlayingDialog from '../dialogs/end-time-is-playing-dialog.vue'
...
const end_timeis_playing_dialog = ref<InstanceType<typeof EndTimeIsPlayingDialog> | null>(null)
...
const {
    ...
    crudRelayHandlers,
} = useTimeIsView({ props, emits, context_menu, end_timeis_playing_dialog })
