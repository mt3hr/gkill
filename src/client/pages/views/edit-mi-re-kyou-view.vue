<template>
    <div style="position: relative; min-height: 100px; flex: 1 1 auto; display: flex; flex-direction: column;">
        ...
        <v-card v-if="cloned_kyou.typed_mirekyou" class="pa-2" variant="flat">
            <v-card-title>
                <v-row class="pa-0 ma-0">
                    <v-col cols="auto" class="pa-0 ma-0">
                        <span>{{ i18n.global.t("EDIT_MI_REKYOU_TITLE") }}</span>
                    </v-col>
                    <v-spacer />
                    <v-col cols="auto" class="pa-0 ma-0">
                        <v-checkbox v-model="show_kyou" :readonly="is_busy"
            ...
            <table>
                <tbody>
                    <tr>
                        <td>
                            <v-select class="select" v-model="mi_board_name" :items="mi_board_names" :readonly="is_busy"
                                :label="i18n.global.t('MI_BOARD_NAME_TITLE')" />
                        </td>
                        <td>
                            <v-btn color="primary" @click="show_new_board_name_dialog()" icon="mdi-plus" dark
                                size="small" :disabled="is_busy"></v-btn>
                        </td>
                    </tr>
                </tbody>
            </table>
                            ...
                            <tr>
                                <td>
                                    <v-btn dark color="secondary" @click="clear_estimate_start_date_time()"
                                        :disabled="is_busy">{{ i18n.global.t("CLEAR_TITLE") }}</v-btn>
                                </td>
                                <td>
                                    <v-btn dark color="primary" @click="now_to_estimate_start_date_time()"
                                        :disabled="is_busy">{{ i18n.global.t("CURRENT_DATE_TIME_TITLE") }}</v-btn>
                                </td>
                            </tr>
                        </tbody>
                    </table>
            ...
            <v-card v-if="show_kyou">
                <KyouView :application_config="application_config" :gkill_api="gkill_api"
                    ...
                    :enable_dialog="enable_dialog" :is_readonly_mi_check="true" :show_rep_name="true"
                    :force_show_latest_kyou_info="true" :show_attached_timeis="true" :show_attached_tags="true"
                    :show_attached_texts="true" :show_attached_notifications="true" :show_update_time="false"
...
const props = defineProps<EditMiReKyouViewProps>()
...
const {
    ...
    // Event relay objects
    crudRelayHandlers,
} = useEditMiReKyouView({ props, emits })
</script>
