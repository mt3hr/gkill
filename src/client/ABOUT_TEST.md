...
Vue 3 フロントエンドのテスト。ユニットテスト（~632テスト）と E2E テスト（202テスト、31ファイル）の合計~834テストで、API クライアント、データモデル、パーサ、Composable、ルーター、i18n、Service Worker、全13ルート + CRUD操作フロー + 設定機能テスト + 回帰テスト + クリップボード保存機能テスト + ダッシュボード機能テストをカバーする。
...
│   ├── datas/               #   データモデル (23ファイル, dashboard-config.test.ts 追加)
│   ├── dnote/               #   D-note モジュール (5ファイル)
│   ├── kftl/                #   KFTL パーサ (5ファイル)
│   ├── composables/         #   Vue Composable (7ファイル, useDashboardPage 追加)
│   ├── router.test.ts       #   ルーター (13ルート, dashboard 追加)
│   ├── i18n-completeness.test.ts  #  i18n 完全性 (7ロケール)
│   └── service-worker.test.ts     #  Service Worker
├── e2e/                     # E2E テスト (202テスト, 31ファイル, dashboard.spec.ts 追加)
...
| データモデル | 23ファイル | 全23データ型の生成・代入・シリアライゼーション（DashboardConfig 追加） |
| D-note | 5ファイル | 述語関数、キー取得、集計ターゲット、アグリゲータ |
| KFTL パーサ | 5ファイル | 構文解析、型判定、リクエスト生成 |
| Composable | 7ファイル | 追加/編集ビュー、コンテキストメニュー、削除確認、クリップボード保存、useDashboardPage |
| ルーター | 1ファイル | 13ルートの定義と遷移（dashboard 追加） |
