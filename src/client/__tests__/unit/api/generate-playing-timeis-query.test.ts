...
// generate_playing_timeis_query が読むフィールドだけを持つ最小のApplicationConfig。
...
function make_application_config(overrides: Record<string, unknown> = {}): ApplicationConfig {
    return {
        ...
        playing_timeis_json_data: undefined,
        rep_struct: {
            rep_name: '', children: [
...
// PlayingTimeIsConfig.parse が受け取る形のJSONを実クラスの直列化で作る。
...
function make_saved_query_json(setup: (query: FindKyouQuery) => void): Record<string, unknown> {
    ...
    setup(query)
    return { playing_timeis_find_kyou_query: JSON.parse(JSON.stringify(query)) }
}

describe('generate_playing_timeis_query', () => {
    const playing_time = new Date('2026-08-09T12:00:00Z')

    test('application_config が null なら基礎クエリのみ(従来のバグ互換)', () => {
        const query = generate_playing_timeis_query(null, playing_time)
        // 非nullの playing_time が実行中検索を表す
        expect(query.playing_time).toBe(playing_time)
    ...
    test('記録タイプはカスタム条件の有無によらずTimeIs固定', () => {
        for (const config of [null, make_application_config(), make_application_config({
            playing_timeis_json_data: make_saved_query_json(saved => { saved.words = [] }),
        })]) {
            const query = generate_playing_timeis_query(config, playing_time)
    ...
    test('for_share_kyou ならrep/tag構造にもカスタム条件にも触れない', () => {
        ...
        const config = {
            for_share_kyou: true,
            playing_timeis_json_data: make_saved_query_json(saved => {
                saved.words = []
    ...
    test('未設定なら従来の既定動作(全rep + タグフィルタ未使用、非表示タグなし)', () => {
        const query = generate_playing_timeis_query(make_application_config(), playing_time)
        expect(query.playing_time).toBe(playing_time)
        expect(query.tags).toBeNull()
    ...
    test('保存クエリが明示的にnullでも既定動作', () => {
        const config = make_application_config({
            playing_timeis_json_data: { playing_timeis_find_kyou_query: null },
        })
        const query = generate_playing_timeis_query(config, playing_time)
        expect(query.reps).toEqual(['Kmemo_device1_all', 'TimeIs_device1_all', 'TimeIs_nested_all'])
        expect(query.tags).toBeNull()
    ...
    test('保存クエリ由来の playing_time は常に強制上書きされる', () => {
        const config = make_application_config({
            playing_timeis_json_data: make_saved_query_json(saved => {
                saved.playing_time = new Date('2000-01-01T00:00:00Z')
            }),
        })
        const query = generate_playing_timeis_query(config, playing_time)
        expect(query.playing_time).toBe(playing_time)
