...
import type { PlayingTimeIsViewProps } from '@/pages/views/playing-time-is-view-props'
import type { PlayingTimeIsViewEmits } from '@/pages/views/playing-time-is-view-emits'
...
export function usePlayingTimeIsView(options: {
    props: PlayingTimeIsViewProps,
    emits: PlayingTimeIsViewEmits,
}) {
    const { props, emits } = options

    // ── Template refs ──
    const playing_timeis_root = ref<HTMLElement | null>(null)
    ...
    // 実行中(playing)の一覧は1つだけ。rykv / mi と違って列を持たない
    ...
    async function search(update_cache: boolean): Promise<void> {
        ...
        // 検索する。Tickでまとめる
        query.value = generate_get_playing_timeis_kyous_query(last_added_request_time.value)
        try {
    ...
    useScopedEnterForKFTL(playing_timeis_root, show_kftl_dialog, enable_enter_shortcut)
    useScopedCtrlVForClipboard(playing_timeis_root, show_save_clipboard_to_file_dialog, enable_enter_shortcut)
