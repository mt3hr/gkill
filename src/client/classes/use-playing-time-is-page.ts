...
export function usePlayingTimeIsPage() {
    ...
    // ── Template refs ──
    const playing_timeis_view = ref<ComponentRef | null>(null)
    ...
    function onPlayingViewReceivedErrors(errors: Array<GkillError>): void {
        write_errors(errors)
    }

    function onPlayingViewReceivedMessages(messages: Array<GkillMessage>): void {
        write_messages(messages)
    }

    function onPlayingViewDeletedKyou(): void {
        // no-op
    }

    function onPlayingViewDeletedTag(): void {
        // no-op
    }

    function onPlayingViewDeletedText(): void {
        // no-op
    }

    function onPlayingViewDeletedNotification(): void {
        // no-op
    }

    function onPlayingViewRegisteredKyou(registered_kyou: Kyou): void {
        check_mi_board_update(registered_kyou)
    }

    function onPlayingViewRegisteredTag(registered_tag: Tag): void {
        check_tag_update(registered_tag)
    }

    function onPlayingViewRegisteredText(): void {
        // no-op
    }

    function onPlayingViewRegisteredNotification(): void {
        // no-op
    }

    function onPlayingViewUpdatedKyou(updated_kyou: Kyou): void {
        check_mi_board_update(updated_kyou)
    }

    // KFTL/MKFL はタグを registered_tag で上げてこないので、保存完了で両方取り直す
    function onPlayingViewSavedKyouByKftl(): void {
        resync_structs()
    }

    function onPlayingViewUpdatedTag(updated_tag: Tag): void {
        check_tag_update(updated_tag)
    }

    function onPlayingViewUpdatedText(): void {
        // no-op
    }

    function onPlayingViewUpdatedNotification(): void {
        // no-op
    }
    ...
    return {
        ...
        navigate_to_page,
        onPlayingViewReceivedErrors,
        onPlayingViewReceivedMessages,
        onPlayingViewDeletedKyou,
        onPlayingViewDeletedTag,
        onPlayingViewDeletedText,
        onPlayingViewDeletedNotification,
        onPlayingViewRegisteredKyou,
        onPlayingViewRegisteredTag,
        onPlayingViewRegisteredText,
        onPlayingViewRegisteredNotification,
        onPlayingViewUpdatedKyou,
        onPlayingViewSavedKyouByKftl,
        onPlayingViewUpdatedTag,
        onPlayingViewUpdatedText,
