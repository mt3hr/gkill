...
- **Aggregate Targets**: カウント、合計、平均、最小、最大など。`dnote-agregate-target/` の21ファイルのうち19種が集計対象（残り2つは `average-info.ts` / `time-of-day-average-info.ts` のヘルパ）
...
| `dnote-agregate-target/` | 21 | `aggregate-targets.test.ts` |
