...
import type EndTimeIsPlayingDialog from '@/pages/dialogs/end-time-is-playing-dialog.vue'
...
export function useTimeIsView(options: {
    ...
    end_timeis_playing_dialog: ReturnType<typeof ref<InstanceType<typeof EndTimeIsPlayingDialog> | null>>,
}) {
    const { props, emits, context_menu, end_timeis_playing_dialog } = options

    // ── Lifecycle ──
    // 表示時点で再生中TimeIsを最新化しておき、終了操作時の読み込み待ちをなくす。
    onMounted(async () => {
        const timeis = props.kyou.typed_timeis
        if (!props.show_timeis_playing_end_button || !timeis || timeis.end_time) {
            return
        }
        try {
    ...
    function show_end_timeis_dialog(): void {
        end_timeis_playing_dialog.value?.show()
    }
