    ...
    async function execute_save(tag_names: Array<string>): Promise<void> {
        try {
            ...
            const tx = await run_in_tx(props.gkill_api, async (tx_id) => {
                ...
                const tag_result = await add_tags_to_target(props.gkill_api, props.application_config, new_lantana.id, tag_names, tx_id)
            ...
            const added_kyou = await fetch_committed_kyou(props.gkill_api, new_lantana.id)
            if (added_kyou) {
