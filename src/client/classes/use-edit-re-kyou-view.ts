...
export function useEditReKyouView(options: {
     ...
     * 関連日時が変わっているか。
     *
     * タグだけを足したときにこれが偽なら update_rekyou は呼ばない。
     * 呼ぶと中身の同じ新しい版が1つ増えてしまう
     */
    function is_body_changed(): boolean {
        const rekyou = cloned_kyou.value.typed_rekyou
        if (!rekyou) {
            return false
        }
        return moment(rekyou.related_time).toDate().getTime() !== moment(related_date_string.value + " " + related_time_string.value).toDate().getTime()
    }
