...
import type { EditPlayingTimeIsDialogProps } from '@/pages/dialogs/edit-playing-time-is-dialog-props'
import type { EditPlayingTimeIsDialogEmits } from '@/pages/dialogs/edit-playing-time-is-dialog-emits'
import { close_dialog_via_history, useDialogHistoryStack } from '@/classes/use-dialog-history-stack'
import { useFloatingDialog } from '@/classes/use-floating-dialog'
import { FindKyouQuery } from '@/classes/api/find_query/find-kyou-query'

export function useEditPlayingTimeIsDialog(options: {
    props: EditPlayingTimeIsDialogProps
    emits: EditPlayingTimeIsDialogEmits
}) {
    const { props, emits } = options

    const is_show_dialog: Ref<boolean> = ref(false)
    useDialogHistoryStack(is_show_dialog)
    const ui = useFloatingDialog("edit-playing-time-is-dialog", {
        centerMode: "always",
    })
    ...
    const is_use_custom_find_kyou_query = computed<boolean>({
        ...
        set: (value: boolean) => {
            ...
            if (current_query.value === null) {
                current_query.value = FindKyouQuery.generate_default_query_for_playing_timeis(props.application_config)
            }
        },
    })
    ...
    function open_query_editor(): void {
            ...
            const initial = current_query.value ?? FindKyouQuery.generate_default_query_for_playing_timeis(props.application_config)
    ...
    function emit_current_config(): void {
            ...
            config.playing_timeis_find_kyou_query = current_query.value
