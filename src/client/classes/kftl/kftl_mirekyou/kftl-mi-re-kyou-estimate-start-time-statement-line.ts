...
import { KFTLMiReKyouEstimateEndTimeStatementLine } from './kftl-mi-re-kyou-estimate-end-time-statement-line'
import { KFTLMiReKyouTagStatementLine } from './kftl-mi-re-kyou-tag-statement-line'

// リポストタスクの見積開始日時行。「？」/「?」を付けると行エラーになる
export class KFTLMiReKyouEstimateStartTimeStatementLine extends KFTLStatementLine {
    ...
    constructor(line_text: string, context: KFTLStatementLineContext, prev_line_is_meta_info: boolean) {
        ...
        context.set_next_statement_line_constructor(KFTLMiReKyouTagStatementLine.generate_next_constructor(context.get_next_statement_line_text(), prev_line_is_meta_info, (line_text: string, context: KFTLStatementLineContext) => new KFTLMiReKyouEstimateEndTimeStatementLine(line_text, context, prev_line_is_meta_info)))
    }
