...
export function usePlaingTimeIsPage() {
    ...
    function onPlaingViewDeletedTag(): void {
        // no-op
    }

    function onPlaingViewDeletedText(): void {
        // no-op
    }

    function onPlaingViewDeletedNotification(): void {
        // no-op
    }

    function onPlaingViewRegisteredKyou(registered_kyou: Kyou): void {
    ...
    function onPlaingViewRegisteredNotification(): void {
        // no-op
    }

    function onPlaingViewUpdatedKyou(updated_kyou: Kyou): void {
        check_mi_board_update(updated_kyou)
    }

    // KFTL/MKFL はタグを registered_tag で上げてこないので、保存完了で両方取り直す
    function onPlaingViewSavedKyouByKftl(): void {
        resync_structs()
    }
    ...
    function onPlaingViewUpdatedNotification(): void {
        // no-op
    }
    ...
    function url_base64_to_uint8_array(base64_string: string) {
        ...
        const base64 = (base64_string + padding).replace(/\-/g, '+').replace(/_/g, '/'); const raw_data = window.atob(base64);
