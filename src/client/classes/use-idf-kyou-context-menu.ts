...
export function useIDFKyouContextMenu(options: {
    ...
    // ── Computed ──
    const context_menu_style = computed(() => `{ position: absolute; left: ${Math.min(document.defaultView!.innerWidth - 130, position_x.value.valueOf())}px; top: ${Math.min(Math.max(50, document.defaultView!.innerHeight - ( + 8 + (48 * (9 + (tag_history.value.length > 0 ? 1 : 0) + (props.application_config.session_is_local ? 2 : 0) + (props.kyou.typed_idf_kyou?.is_zip ? 1 : 0))))), position_y.value.valueOf())}px; }`)
