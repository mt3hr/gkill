...
import type { IDFKyouContextMenuProps } from '@/pages/views/idf-kyou-context-menu-props'
import type { KyouViewEmits } from '@/pages/views/kyou-view-emits'

export function useIDFKyouContextMenu(options: {
    props: IDFKyouContextMenuProps,
    emits: KyouViewEmits,
}) {
    ...
    // ── Computed ──
    const context_menu_style = computed(() => `{ position: absolute; left: ${Math.min(document.defaultView!.innerWidth - 130, position_x.value.valueOf())}px; top: ${Math.min(Math.max(50, document.defaultView!.innerHeight - ( + 8 + (48 * (10 + (tag_history.value.length > 0 ? 1 : 0) + (props.application_config.session_is_local ? 2 : 0) + (props.kyou.typed_idf_kyou?.is_zip ? 1 : 0))))), position_y.value.valueOf())}px; }`)
    ...
    async function copy_id(): Promise<void> {
        ...
        message.message_code = GkillMessageCodes.copied_idf_kyou_id
    ...
    async function show_edit_idf_kyou_dialog(): Promise<void> {
        emits('requested_open_rykv_dialog', 'edit_idf_kyou', props.kyou)
    }
