...
|--------------|---------|----------------|---------------|
| Go バックエンド (`server/`) | 1405 | 211 | Go `testing` |
| フロントエンド ユニット (`client/`) | 2089 | 185 | Vitest |
| フロントエンド E2E (`client/`) | 255 | 47 | Playwright |
| MCP サーバ (`server/gkill/mcp/`) | 1162 | 43 | Go `testing` |
| ツール (`tools/`) | 83 | 3 | Vitest |
| Android (`android/`) | 26 | 2 | JUnit 4 |
| Wear OS (`wear_os/`) | 232 | 18 | JUnit 4 + MockK |
| **合計** | **5,252** | **509** | |
...
| ディレクトリ | テスト仕様 | 概要 |
|-------------|-----------|------|
| `client/` | [client/ABOUT_TEST.md](client/ABOUT_TEST.md) | フロントエンド全体（unit 2089 + E2E 255） |
...
| `server/` | [server/ABOUT_TEST.md](server/ABOUT_TEST.md) | Go バックエンド全体（1405テスト / 33パッケージ） |
...
| `server/gkill/mcp/` | [server/gkill/mcp/ABOUT_TEST.md](server/gkill/mcp/ABOUT_TEST.md) | MCP サーバ（1162テスト）。コミット済みゴールデンとのバイト一致（採取時は旧 Node 実装の出力）を含む |
