...
|--------------|---------|----------------|---------------|
| Go バックエンド (`server/`) | 1227 | 178 | Go `testing` |
| フロントエンド ユニット (`client/`) | 2057 | 172 | Vitest |
...
`src/plugins/` の Go テスト 152件は独立モジュールのため上表（`src/server` 基準の集計）には含まれない。実行は `npm run test_plugins` が担当し、`npm test` からも呼ばれる（[plugins/ABOUT_TEST.md](plugins/ABOUT_TEST.md) 参照）。
...
- **手書きの一覧はソースから機械的に導出する** — 例: エラーコードの重複チェックは、手書きした29個だけでなく `go/parser` で読み取った全定数（導入時406個・現在414個）が対象になった
...
| ディレクトリ | テスト仕様 | 概要 |
|-------------|-----------|------|
| `client/` | [client/ABOUT_TEST.md](client/ABOUT_TEST.md) | フロントエンド全体（unit 2057 + E2E 251） |
...
| `server/` | [server/ABOUT_TEST.md](server/ABOUT_TEST.md) | Go バックエンド全体（1227テスト / 32パッケージ） |
...
| `server/gkill/plugin/sdk/` | [server/gkill/plugin/sdk/ABOUT_TEST.md](server/gkill/plugin/sdk/ABOUT_TEST.md) | プラグイン SDK（stdio ループ + EnsureConfig + ZIP走査 + キャッシュDBパス + ワード判定、47テスト） |
