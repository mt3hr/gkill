<template>
    <v-menu v-model="is_show" :style="context_menu_style">
        <v-list class="gkill_context_menu_list">
            ...
            <v-list-item @click="show_edit_kc_dialog()">
    ...
    <AddTagDialog :application_config="application_config" :gkill_api="gkill_api" :highlight_targets="highlight_targets"
        ...
        @requested_reload_kyou="(...kyou: any[]) => emits('requested_reload_kyou', kyou[0] as Kyou)"
        @requested_reload_list="emits('requested_reload_list')"
        @received_messages="(...messages :any[]) => emits('received_messages', messages[0] as Array<GkillMessage>)" ref="add_tag_dialog" />
...
import type { KCContextMenuProps } from './kc-context-menu-props'
...
const props = defineProps<KCContextMenuProps>()
const emits = defineEmits<KyouViewEmits>()
defineExpose({ show })

const edit_kc_dialog = ref<InstanceType<typeof EditKCDialog> | null>(null);
...
async function copy_id(): Promise<void> {
    ...
    message.message_code = GkillMessageCodes.copied_kc_id
...
async function show_edit_kc_dialog(): Promise<void> {
    edit_kc_dialog.value?.show()
}
