<template>
    <v-menu v-model="is_show" :style="context_menu_style">
        <v-list class="gkill_context_menu_list">
            ...
            <v-list-item @click="show_edit_mi_dialog()">
    ...
    <EditMiDialog :application_config="application_config" :gkill_api="gkill_api" :highlight_targets="highlight_targets"
        :enable_context_menu="enable_context_menu" :enable_dialog="enable_dialog" :kyou="kyou"
    ...
    <ConfirmDeleteKyouDialog :application_config="application_config" :gkill_api="gkill_api"
        ...
        @requested_reload_list="emits('requested_reload_list')"
        @received_errors="(...errors: any[]) => emits('received_errors', errors[0] as Array<GkillError>)"
        @received_messages="(...message: any[]) => emits('received_messages', message[0] as Array<GkillMessage>)"
...
import type { MiContextMenuProps } from './mi-context-menu-props'
...
const edit_mi_dialog = ref<InstanceType<typeof EditMiDialog> | null>(null);
...
const props = defineProps<MiContextMenuProps>()
...
async function copy_id(): Promise<void> {
    ...
    message.message_code = GkillMessageCodes.copied_mi_id
...
async function show_edit_mi_dialog(): Promise<void> {
    edit_mi_dialog.value?.show()
}
