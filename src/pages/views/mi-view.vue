                        ...
                        <v-card>
                            ...
                            <KyouListView :kyou_height="56 + 25" :width="400"
                                ...
                                :is_readonly_mi_check="false" :show_checkbox="false" :show_footer="true" @scroll_list="(scroll_top: number) => {
                                    ...
                                    GkillAPI.get_instance().set_saved_mi_scroll_indexs(match_kyous_list_top_list)
                                }" @clicked_list_view="() => {
...
async function init(): Promise<void> {
    return nextTick(async () => {
        ...
        try {
            ...
            // スクロール位置の復元
            match_kyous_list_top_list.value = GkillAPI.get_instance().get_saved_mi_scroll_indexs()

            // 前回開いていた列があれば復元する
            skip_search_this_tick.value = true
            const saved_querys = GkillAPI.get_instance().get_saved_mi_find_kyou_querys()
...
async function search(column_index: number, query: FindKyouQuery, force_search?: boolean, update_cache?: boolean): Promise<void> {
    ...
    try {
        ...
        GkillAPI.get_instance().set_saved_mi_find_kyou_querys(querys.value)
...
async function close_list_view(column_index: number): Promise<void> {
    ...
    GkillAPI.get_instance().set_saved_mi_find_kyou_querys(querys.value)
    GkillAPI.get_instance().set_saved_mi_scroll_indexs(match_kyous_list_top_list.value)
}
