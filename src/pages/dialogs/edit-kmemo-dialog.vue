<template>

</template>
<script setup lang="ts">

// ˅
'use strict';

import { EditKmemoDialogProps } from './edit-kmemo-dialog-props';
import { EditKmemoVIew } from '../views/edit-kmemo-v-iew';
import { KyouDialogEmits } from '../views/kyou-dialog-emits';
import { KyouView } from '../views/kyou-view.vue';
import { KyouViewEmits } from '../views/kyou-view-emits';

// ˄

export class EditKmemoDialog {
    // ˅
    
    // ˄

    private props: EditKmemoDialogProps;

    private kyouView: KyouView;

    private emits: KyouViewEmits;

    private emits: KyouDialogEmits;

    private editKmemoVIew: EditKmemoVIew;

    async show(): Promise<void> {
        // ˅
        throw new Error('Not implemented');
        // ˄
    }

    async hide(): Promise<void> {
        // ˅
        throw new Error('Not implemented');
        // ˄
    }

    // ˅
    
    // ˄
}

// ˅

// ˄

</script>
<style scoped></style>