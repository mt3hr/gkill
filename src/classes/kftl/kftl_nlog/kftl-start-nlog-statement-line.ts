...
export class KFTLStartNlogStatementLine extends KFTLStatementLine {
    ...
    constructor(line_text: string, context: KFTLStatementLineContext) {
        ...
        context.set_next_statement_line_constructor((line_text: string, context: KFTLStatementLineContext) => new KFTLNlogShopNameStatementLine(line_text, context))
    }

    async apply_this_line_to_request_map(request_map: KFTLRequestMap): Promise<void> {
        const req = new KFTLNlogRequest(this.get_context().get_this_statement_line_target_id(), this.get_context())
