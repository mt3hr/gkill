...
export class KFTLKmemoRequest extends KFTLRequest {
    ...
    override async do_request(gkill_api: GkillAPI, application_config: ApplicationConfig): Promise<Array<GkillError>> {
        ...
        req.kmemo.create_device = application_config.device
        req.kmemo.create_time = now
        req.kmemo.create_user = application_config.user_id
        req.kmemo.update_app = "gkill_kftl"
        req.kmemo.update_device = application_config.device
        req.kmemo.update_time = now
        req.kmemo.update_user = application_config.user_id
