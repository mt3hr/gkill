...
func TestBuildOnceRecordsFailureInCacheMeta(t *testing.T) {
	...
	buildCacheFn = func(string, expandedSource) error { return errors.New("boom") }
	if err := buildOnce(pluginDir, expandedSource{}); err == nil || err.Error() != "boom" {
	...
	logs.Reset()
	buildCacheFn = func(string, expandedSource) error {
		c.setMeta("build_state", "idle")
		return nil
	}
	if err := buildOnce(pluginDir, expandedSource{}); err != nil {
