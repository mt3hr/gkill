...
func TestCacheDBPath_UsesGkillCacheDir(t *testing.T) {
	...
	want := filepath.Join(home, "caches", "plugin_cache", "testuser", "gkill_plugin_claudecode", "cache.db")
	if got != want {
...
func TestCacheDBPath_FallsBackToPluginDir(t *testing.T) {
	...
	pluginDir := filepath.Join(t.TempDir(), "somewhere", "gkill_plugin_claudecode")
	got := cacheDBPath(pluginDir)
