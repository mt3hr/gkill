...
func TestCacheDBPath_UsesGkillCacheDir(t *testing.T) {
	...
	pluginDir := filepath.Join(home, "plugins", "testuser", "gkill_plugin_fitbit")
	got := cacheDBPath(pluginDir)

	want := filepath.Join(home, "caches", "plugin_cache", "testuser", "gkill_plugin_fitbit", "cache.db")
	if got != want {
...
func TestCacheDBPath_FallsBackToPluginDir(t *testing.T) {
	...
	pluginDir := filepath.Join(t.TempDir(), "somewhere", "gkill_plugin_fitbit")
	got := cacheDBPath(pluginDir)
