...
func TestBuildOnceRecordsFailureInCacheMeta(t *testing.T) {
	...
	buildCacheFn = func(context.Context, string, pluginConfig) error { return errors.New("boom") }
	if err := buildOnce(context.Background(), pluginDir, pluginConfig{}); err == nil || err.Error() != "boom" {
	...
	logs.Reset()
	buildCacheFn = func(context.Context, string, pluginConfig) error {
		c.setMeta("build_state", "idle")
		return nil
	}
	if err := buildOnce(context.Background(), pluginDir, pluginConfig{}); err != nil {
