...
// maxShownRepos は設定画面に並べるリポジトリの最大件数。
const maxShownRepos = 100

// renderConfigHTML は設定画面のHTMLを返す。
//
// キャッシュの状態(cache_meta と repo 表)しか読まない。ここで zip を開いてはいけない ――
...
func renderConfigHTML(pluginDir string, stats cacheStats, patterns []string) string {
	var sb strings.Builder
	sb.WriteString(configHTMLHead)
	sb.WriteString(`<h2>アーカイブ Git コミットログプラグイン</h2>`)

	if stats.CommitCount > 0 {
		sb.WriteString(`<div class="ok">`)
		fmt.Fprintf(&sb, `<p>✓ <strong>%d リポジトリ・%d コミット</strong>を読み込んでいます</p>`, stats.RepNameCount, stats.CommitCount)
		sb.WriteString(`</div>`)
	} else {
		sb.WriteString(`<div class="warn">`)
		sb.WriteString(`<p>まだ読み込まれていません。</p>`)
		sb.WriteString(`<p>Git リポジトリを固めた zip の実パスか、zip を置いたフォルダを、下の手順で指定してください。</p>`)
	...
	fmt.Fprintf(&sb, `<tr><td class="k">対象リポジトリ数</td><td>%d</td></tr>`, stats.TargetRepoCount)
	fmt.Fprintf(&sb, `<tr><td class="k">取り込み済みリポジトリ</td><td>%d</td></tr>`, stats.RepoCount)
	fmt.Fprintf(&sb, `<tr><td class="k">rep 名の数</td><td>%d</td></tr>`, stats.RepNameCount)
	fmt.Fprintf(&sb, `<tr><td class="k">コミット数</td><td>%d</td></tr>`, stats.CommitCount)
	...
	if stats.StatsErrorCount > 0 {
		fmt.Fprintf(&sb, `<div class="warn"><p>行数を集計できなかったコミットが <strong>%d 件</strong>あります（コミット自体は表示されます）。</p></div>`, stats.StatsErrorCount)
	}
	...
	if skipped > 0 {
		sb.WriteString(`<div class="warn"><p>読めなかったリポジトリ:</p><ul>`)
		for _, repo := range stats.Repos {
			if repo.Note == "" {
				continue
			}
			sb.WriteString(`<li><code>` + html.EscapeString(repo.ArchivePath) + `!/` + html.EscapeString(repo.GitDir) + `</code> ` + html.EscapeString(repo.Note) + `</li>`)
		}
		sb.WriteString(`</ul></div>`)
	}
	if stats.BuildError != "" {
	...
	if len(stats.Repos) > 0 {
		fmt.Fprintf(&sb, `<h3>取り込んだリポジトリ (%d)</h3><ul>`, len(stats.Repos))
		for i, repo := range stats.Repos {
			if i >= maxShownRepos {
				fmt.Fprintf(&sb, `<li>ほか %d 件</li>`, len(stats.Repos)-i)
				break
			}
			fmt.Fprintf(&sb, `<li><strong>%s</strong> — %d コミット <code>%s!/%s</code></li>`,
				html.EscapeString(repo.RepName), repo.CommitCount,
				html.EscapeString(repo.ArchivePath), html.EscapeString(repo.GitDir))
		}
		sb.WriteString(`</ul>`)
	}
	...
	sb.WriteString(`<textarea id="gkill_source_dirs" rows="6" spellcheck="false">` +
	...
	sb.WriteString(`<div class="hint">` +
		...
		`zip の実パスを書くとその zip だけを、フォルダを書くと配下の <code>*.zip</code> を再帰的に読みます。` +
		`ワイルドカード <code>*</code> <code>**</code> <code>?</code> <code>[]</code> と、` +
		`先頭の <code>~</code> と環境変数(<code>$HOME</code> など)も展開されます` +
		`(ただしgkillをWindowsサービスで動かしている場合は実行アカウントのホームになるため、絶対パスが確実です)。` +
		`zip は展開せず中の <code>.git</code> だけを読むので、<code>.git</code> の無い zip は読み飛ばします。` +
		`同じリポジトリが複数の zip に入っていても、コミットは1件ずつです。` +
		`<code>max_git_dir_mb</code> は1リポジトリの <code>.git</code> の合計サイズの上限(MB)です。` +
...
func renderBuildProgress(sb *strings.Builder, stats cacheStats) {
	...
	switch stats.BuildState {
	case "scanning":
		sb.WriteString(`<p>zip を走査しています…</p>`)
	case "ingesting":
		fmt.Fprintf(sb, `<p>取り込み中 %d / %d リポジトリ</p>`, stats.BuildDone, stats.BuildTotal)
	default:
