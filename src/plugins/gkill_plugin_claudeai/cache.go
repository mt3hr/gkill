...
// ビルダ以外から呼ばないこと。
//
// Claude.ai のエクスポートは ZIP の中の conversations.json が丸ごと入れ替わる形なので、
...
func (c *pluginCache) build(pluginDir string, patterns []string) error {
	...
	if len(entries) == 0 {
		...
		return fmt.Errorf("conversations.json を含む ZIP（conversations-000.zip など）が見つかりません。Claude.ai からエクスポートした ZIP を解凍せずにデータソースのフォルダへ置いてください")
	}
