...
func TestBuildOnceRecordsFailureInCacheMeta(t *testing.T) {
	...
	if err := buildOnce(pluginDir, pluginConfig{}); err == nil || err.Error() != "boom" {
	...
	logs.Reset()
	buildCacheFn = func(string, pluginConfig) error {
		c.setMeta("build_state", "idle")
		return nil
	}
	if err := buildOnce(pluginDir, pluginConfig{}); err != nil {
