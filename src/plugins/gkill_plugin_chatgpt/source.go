...
// 取り込み元は ChatGPT からダウンロードした ZIP そのもの。**展開済みの JSON は読まない。**
...
const extractedFolderMessage = "ZIP ではないので読みません（展開済みの JSON は読みません）。" +
	"ChatGPT からエクスポートした ZIP を解凍せずに置き、その ZIP かフォルダを指定してください。"

// relevantProblems は設定画面に出す問題だけを残す。
//
//   - nested_zip / mixed_exports は Takeout 固有の話（ChatGPT の ZIP に世代の概念は無い）
...
//     文言を ChatGPT 向けに言い換えて残す。旧配置のまま黙って0件にしないための唯一の出口
...
// ChatGPT の ZIP も書き出し時刻しか入らないので、中身が変わっても動かない（ADR-0303）。
