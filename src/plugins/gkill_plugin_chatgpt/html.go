...
func renderConfigHTML(pluginDir string, stats cacheStats, patterns []string, src sdk.ExpandedSource) string {
	...
	sb.WriteString(`<h2>ChatGPT チャット履歴プラグイン</h2>`)
	...
	if stats.MessageCount > 0 {
		...
		sb.WriteString(`<p>データを更新するには、ChatGPT から再エクスポートして ZIP を置き換えてください` +
		...
		sb.WriteString(`<li>ChatGPT の <strong>Settings</strong> → <strong>Data controls</strong> → ` +
			`「<strong>Export data</strong>」をクリック</li>`)
		sb.WriteString(`<li>ZIPが届いたら<strong>解凍せず</strong>、そのまま下の「データソース」で指定したフォルダに置く</li>`)
		sb.WriteString(`<li>ZIP の中の <code>conversations-000.json</code> などの分割ファイル` +
			`（無ければ <code>conversations.json</code>）を自動で読みます。解凍した JSON は読みません</li>`)
	...
	sb.WriteString(`<div class="hint">` +
		...
		`マッチしたフォルダは再帰的に走査して <code>*.zip</code> を探し、ZIP の中の <code>conversations-*.json</code>` +
		`(無ければ <code>conversations.json</code>)を読みます。ZIP を直接指定してもかまいません。` +
