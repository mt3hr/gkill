...
func renderConfigHTML(pluginDir string, patterns []string, src expandedSource) string {
	...
	if err != nil {
		...
		sb.WriteString(`<li>ZIPが届いたら解凍する</li>`)
		sb.WriteString(`<li><code>conversations-000.json</code> などの分割ファイル、` +
			`または <code>conversations.json</code> を取り出す</li>`)
		...
		sb.WriteString(`<p>データを更新するには、ChatGPT から再エクスポートしてファイルを置き換えてください。</p>`)
		sb.WriteString(`</div>`)
	}
	...
	sb.WriteString(`<div class="hint">` +
		...
		`マッチしたフォルダは再帰的に走査して <code>conversations-*.json</code>` +
		`(無ければ <code>conversations.json</code>)を探し、マッチしたファイルはそのまま読みます。` +
