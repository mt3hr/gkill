...
func (c *pluginCache) GetMessages(pluginDir string) ([]cachedMessage, error) {
	...
	// 焼くと既存キャッシュの作り直しが要る）。
	rows, err := db.Query(`SELECT m.msg_id, m.conv_id, m.sender, m.text, m.related_time_unix, m.create_time_unix,
		       COALESCE(c.title, '')
		FROM msg_cache m
	...
	for rows.Next() {
		var m cachedMessage
		if err := rows.Scan(&m.MsgID, &m.ConvID, &m.Sender, &m.Text, &m.RelatedTimeUnix, &m.CreateTimeUnix, &m.ConvTitle); err != nil {
			continue
		}
		msgs = append(msgs, m)
	}
...
func (c *pluginCache) GetMsgByID(pluginDir string, msgID string) (convTitle string, msg cachedMessage, err error) {
	...
	if e := row.Scan(&msg.MsgID, &msg.ConvID, &msg.Sender, &msg.Text,
		&msg.RelatedTimeUnix, &msg.CreateTimeUnix, &convTitle); e != nil {
...
func (c *pluginCache) GetConvForMsg(pluginDir string, msgID string) (convTitle string, msgs []cachedMessage, err error) {
	...
	rows, e := db.Query(`SELECT msg_id, conv_id, sender, text, related_time_unix, create_time_unix
...
func (c *pluginCache) ingestConvs(convs []chatGPTConversation, gen int) error {
	db := c.conn()
	tx, err := db.Begin()
	if err != nil {
	...
	for i := range convs {
		...
		if ingestConvHook != nil {
			if err := ingestConvHook(conv.ID); err != nil {
				return err
			}
		}
		if _, err := tx.Exec(
			`INSERT OR REPLACE INTO conv_cache(conv_id, title, create_time_unix, gen) VALUES(?,?,?,?)`,
			conv.ID, conv.Title, int64(conv.CreateTime), gen); err != nil {
			return fmt.Errorf("error at insert conv %s: %w", conv.ID, err)
		}
		for _, msg := range getMessages(conv) {
			if msg.ID == "" {
				continue
			}
			...
			if _, err := tx.Exec(
				`INSERT OR REPLACE INTO msg_cache(msg_id, conv_id, sender, text, related_time_unix, create_time_unix, gen) VALUES(?,?,?,?,?,?,?)`,
				msg.ID, conv.ID, msg.Author.Role, text, createTimeUnix, createTimeUnix, gen); err != nil {
				return fmt.Errorf("error at insert msg %s: %w", msg.ID, err)
			}
		}
	}
