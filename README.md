...
外部と通信するのは、地図の表示・ブックマーク登録時のページ取得・自分で設定したときのMCP接続・
通知(Web Push)を設定したときの配送だけです）  

## ダウンロード・インストール
[gkillダウンロード](https://github.com/mt3hr/gkill/releases/latest)  

以下のファイル名の `<バージョン>` は、リリースページに並んでいる最新の番号（例: `1.1.8`）に読み替えてください。  
...
## 運用・安全
記録を貯め始めたら、まず以下に目を通してください。  

- **バックアップ・復旧**: 手順は [運用ガイド](documents/reverse/operations-guide.md) の「バックアップ・リストア」章にあります。スナップショット方式と停止方式の2通り、復元の検証手順まで載っています。  
- **外部との通信**: 上記のとおり、地図・ブックマーク登録時のページ取得・自分で設定したMCP接続・通知(Web Push)配送だけです。記録本文が勝手に外へ送られることはありません。  
- **スマホ・ウォッチの接続**: Wear OSコンパニオンからサーバへ繋ぐ場合、ローカル(localhost)以外へは HTTPS が必須です（平文HTTPでは資格情報が漏れるため保存時に拒否されます）。  
- **脆弱性の報告**: [SECURITY.md](SECURITY.md) を参照してください。公開Issueには書かないでください。  
